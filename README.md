...
- **已完成**:
  ...
  - 间距线 $n=1$ 对称线代数核心: SymlineTensionRatio (P1/P2 比较引理, $\tilde F_e<0 \iff \rho<1$ 等价, 三正项分解与张力比链 $\rho\le\rho_0$, STRICT 代数核心).
...
- **未完成 (已登记)**: $H^3$ 算符级等距同构 $K_c\colon H^3 \to H^1$ (双射/谱, 需谱论) 与多项式在 $H^1$
  稠密性; $H^s$ 显式正交系的算符级等距与完备性 (谱论/稠密性); MW 引理重证 (周期延拓/零点截断的谱论部分); 比值证明线中转移矩阵到特征值的谱论连接; 间距线 ($n=1$ 定理族, $n \ge 2$ 开关/约化; SymlineTensionRatio 已覆盖 $n=1$ 对称线代数核心 P1/P2/等价性/张力比链, 超越事实 (Lemma ys2 与 $\gamma_0^*$ 定位) 以假设接入、未形式化);
