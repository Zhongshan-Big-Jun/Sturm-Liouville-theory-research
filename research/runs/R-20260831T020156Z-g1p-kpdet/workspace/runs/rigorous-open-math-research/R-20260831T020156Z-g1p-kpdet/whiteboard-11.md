...
The sequence-11 W7 audit returned `PASS`. The `alpha->0` face is exactly
excluded by a uniform empty wedge. Seal before new research. The corrected
near-one boundary inventory still contains the distinct face `alpha->pi`,
potentially coupled to `theta->pi/2`.
