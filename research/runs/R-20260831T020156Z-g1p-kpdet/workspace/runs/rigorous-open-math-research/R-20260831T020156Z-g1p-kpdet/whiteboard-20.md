...
The fresh W14/W15 audit was rejected by the service usage boundary before any
mathematics or artifact returned. Record `NO_RETURN`, preserve both immutable
submissions as `UNREVIEWED`, and seal. The next model action after quota
recovery is exactly one fresh joint audit with the same hash bindings.
