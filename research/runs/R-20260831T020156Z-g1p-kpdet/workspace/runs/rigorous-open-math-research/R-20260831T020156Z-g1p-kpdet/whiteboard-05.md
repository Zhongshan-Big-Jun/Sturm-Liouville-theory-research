...
The fresh sequence-05 audit returned `PASS` on every strict W3 identity and
sign claim. Seal this audit segment. The next segment may open the smallest
mechanism-distinct wave on the audited mass-to-sign bridge: one direct prover
of `G>=0` or a sharp lower bound, and one falsifier of `G>=0` on the complete
exact system. Neither may repeat W1, W2, or W3.
