...
The sequence-08 bounded repair returned `REPAIRED`. It supplies an explicit
moving-switch compactness, min-max spectral convergence, norm-limit, mass
endpoint-separation, and uniform positivity chain. Seal before one fresh
re-audit. The repair is not accepted strict mathematics until that re-audit.
