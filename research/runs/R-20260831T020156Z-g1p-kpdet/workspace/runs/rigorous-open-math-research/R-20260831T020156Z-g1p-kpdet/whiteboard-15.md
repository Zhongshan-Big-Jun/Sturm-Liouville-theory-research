...
The fresh W10/W11 joint audit returned `PASS` with zero errors and zero gaps.
The accepted strict partial package reduces `G<0` exactly to `q>E`, proves
`B<0` on every complete tuple, and isolates the missing common-`beta`
orientation. The exact W11 negative-`G` family is accepted as a mass-defective
no-crossing slice. Seal before one bounded orientation prover and one
