...
The bounded W10/W11 global finite-`m` wave returned two compatible candidate
partial results. W10 reduces `G<0` to `q>E`, proposes an exact `B`-to-`H`
identity, and excludes `B>=0` on the complete mass system. W11 gives an exact
one-parameter negative-`G` spectral-band family trapped in the positive
coefficient orthant and strictly off the mass surface. Seal before one fresh
joint audit; no third solver or repair is authorized first.
