...
The bounded W12/W13 common-`beta` wave returned. W12 proposes a branch-safe
orientation identity, unique acute-branch reconstruction, and strict KP-DET
closure for `c alpha<=pi/2`, hence all complete tuples with `0<c<=1/2`.
W13 returned only bounded numerical evidence and found no counterexample.
Seal before one fresh independent audit; no repair or third solver is
authorized first.
