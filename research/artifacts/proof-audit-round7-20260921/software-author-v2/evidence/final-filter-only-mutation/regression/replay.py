...
def main():
    ...
    # -S suppresses startup hooks; these exact package search directories make
    # NumPy/SciPy available. The gate only allows their individual package roots.
    inherited_pythonpath = environment.get('PYTHONPATH')
    environment['PYTHONPATH'] = os.pathsep.join(sorted({
        str(Path(root).parent) for root in policy['installed_package_roots'].values()}))
    fixture = Base / 'control-fixtures/outside_probe.py'
    results = []
    print('Replay directory: ' + str(run), flush=True)
    for mode in ('normal', 'optimized'):
        flags = ['-B', '-S'] + (['-O'] if mode == 'optimized' else [])
        ...
        for tree, kind, control in cases:
            ...
            argv = [sys.executable, '-B', '-S', str(run / 'regression/capture_run.py'),
    ...
    summary = dict(role='software repair author; independent review pending', label=args.label,
                   ...
                   child_pythonpath=environment['PYTHONPATH'], child_site_startup_disabled=True,
