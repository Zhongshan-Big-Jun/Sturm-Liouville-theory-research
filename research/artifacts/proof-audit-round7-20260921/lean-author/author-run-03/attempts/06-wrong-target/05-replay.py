...
def inspection_source(output, source):
	...
	Lines += ['open Lean Elab Command Meta in', 'run_cmd do', '  let Env := (← getEnv).setExporting false',
		...
		'    let ActualType ← liftTermElabM <| withOptions (fun O => O.setBool `pp.universes true |>.setBool `pp.fullNames true) do',
		'      return (← ppExpr Info.type).pretty',
		'    let Axioms ← collectAxioms Target',
		'    Results := Results.push <| Json.mkObj [',
		'      ("declaration", toJson Target.toString), ("actual_type", toJson ActualType),',
...
def main():
	...
	Parser.add_argument('--resume-positive', type=Path, help='Author recovery only: reuse a completed exact-root execution; default always recompiles')
	Parser.add_argument('--resume-positive-sha256', help='Required frozen manifest hash for author recovery')
	...
	if Args.resume_positive:
		ManifestPath = Args.resume_positive.resolve()
		require(ManifestPath.is_relative_to(BASE), 'Resume input must belong to this author package')
		require(digest(ManifestPath) == Args.resume_positive_sha256, 'Resume manifest identity mismatch')
		Manifest = json.loads(ManifestPath.read_text())
		require(Path(Manifest['project_root']).resolve() == Snapshot, 'Resume project differs')
		require(Manifest['evidence']['contract'] == json.loads((BASE / 'positive-contract.json').read_text()), 'Resume contract differs')
		require(all(digest(Snapshot / N) == H for N,H in Manifest['input_hashes'].items()), 'Resume source/config differs')
		require(all(digest(P) == H for P,H in Manifest['evidence']['compiled_artifacts'].items()), 'Resume object differs')
		require(all(digest(Path(Config['verifier']).parent / N) == H for N,H in Manifest['evidence']['tool_hashes'].items()), 'Resume verifier differs')
		require(all(digest(Path(Manifest['environment']['prefix']) / N) == H for N,H in Manifest['environment']['binary_hashes'].items()), 'Resume runtime differs')
		require(Manifest['environment']['inherited_lean_path'] == Env['LEAN_PATH'], 'Resume search path differs')
		write_json(Output / '02-resumed-positive.json', dict(manifest=str(ManifestPath), sha256=digest(ManifestPath),
			positive_recompiled=False, source_config_runtime_tools_and_objects_unchanged=True,
			import_hashes_to_be_compared_with_fresh_negative_run=True))
	else:
		...
		require(Positive['exit_code'] == 0, 'Exact-root process failed')
		ManifestPath = Output / 'positive/run-manifest.json'
		Manifest = json.loads(ManifestPath.read_text())
	require(Manifest['machine_verification_passed'] and Manifest['exact_root_passed'],
	...
	ImportIdentity = lambda M: {N:(A['path'],A['sha256']) for N,A in M['target']['import_artifacts'].items()}
	require(ImportIdentity(Manifest) == ImportIdentity(NegativeManifest), 'Positive and freshly checked negative external imports differ')
	...
	Summary = dict(status='author_machine_checks_passed', role='local_lean_author', independent_execution='not_performed_by_this_runner',
		positive_reused_for_author_recovery=bool(Args.resume_positive), external_imports_match_fresh_negative_run=True,
