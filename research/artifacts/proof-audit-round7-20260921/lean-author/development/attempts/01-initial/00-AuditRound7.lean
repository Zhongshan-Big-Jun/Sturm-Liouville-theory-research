...
theorem normalized_mode_has_deriv (n : ℕ) (x : ℝ) :
  ...
  convert (((hasDerivAt_id x).const_mul ((n : ℝ) * Real.pi)).sin).const_mul
    (Real.sqrt 2) using 1 <;> simp [normalized_mode, normalized_mode_slope] <;> ring
...
theorem wronskian_product_to_sum (n : ℕ) (x : ℝ) :
  ...
  have hTrig := trig_product_to_sum (n : ℝ) (Real.pi * x)
  unfold wronskian normalized_mode normalized_mode_slope
  push_cast
  simp only [mul_assoc] at hTrig ⊢
  nlinarith [hSqrt]
...
theorem sin_square_sum_pos (n : ℕ) (hn : 0 < n) (t : ℝ) (ht : 0 < t)
  ...
  have hTerm : 0 < Real.sin (((0 : ℕ) : ℝ) + 1 |>.mul t) ^ 2 := by
