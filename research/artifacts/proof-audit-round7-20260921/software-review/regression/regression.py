...
def main():
	...
	Bindings={K:str(getattr(V,'__file__','')) for K,V in sys.modules.items() if getattr(V,'__file__',None) and Path(V.__file__).resolve().is_relative_to(Base)}
	Summary=dict(tree=Args.tree,python=sys.version,executable=sys.executable,platform=platform.platform(),optimize=sys.flags.optimize,numpy=np.__version__,scipy=scipy.__version__,tests=len(Rows),passed=sum(R['passed'] for R in Rows),failed=sum(not R['passed'] for R in Rows),source_sha256=Sources,module_files=Bindings,loader=LoadLog,expressions=ExpressionLog,checks=Rows)
