...
def run_checks():
	...
	# Other current documents are coordinator-owned and can change concurrently.
	# Keep exact byte observations, but enforce immutability only for frozen inputs/R5.
	Protected = [Name for Name in Input["files"] if Name.startswith("research/artifacts/") or Name=="docs/SL_cofinite_left_definite.tex"]
	for Name in Protected:
		Digest = hashlib.sha256((Root/Name).read_bytes()).hexdigest()
		check_true("protected input unchanged: "+Name,Digest==Input["files"][Name]["sha256"],Static)
	A7 = "runs/three-arm-pilot-v2/pilot-v6-hs-domain/arms/c-qed/output/proof.md"
	check_true("historical A7 body unchanged",hashlib.sha256((Root/A7).read_bytes()).hexdigest()=="daf055b84e09024f6a653b57adb771e50b268e1cf9692dab97c6ab59a7bd9987",Static)
	Criteria = "docs/SL_denseness_criteria.tex"
	CriteriaBaseline = subprocess.check_output(["git","show","92d46e99e36cc0a74e8f7bb13430749c222ee54f:"+Criteria],cwd=Root)
	AlgebraStart = CriteriaBaseline.index("其代数线性包为".encode())
	AlgebraEnd = CriteriaBaseline.index(r"\begin{remark}".encode(),AlgebraStart)
	check_true("referenced criteria algebra paragraph unchanged despite recorded source drift",CriteriaBaseline[AlgebraStart:AlgebraEnd] in (Root/Criteria).read_bytes(),Static)
	Observations = {Name:{"initial_sha256":Info["sha256"],"current_sha256":hashlib.sha256((Root/Name).read_bytes()).hexdigest()} for Name,Info in Input["files"].items() if Name not in Protected and Name!="docs/SL_fractional_left_definite.tex"}
	return {
		...
		"checks":Checks,
		"coordinator_owned_current_source_observations":Observations,
		"limits":[
