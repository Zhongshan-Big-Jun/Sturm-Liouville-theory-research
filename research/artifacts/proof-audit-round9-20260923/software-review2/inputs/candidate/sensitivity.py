...
def main():
	...
	# Follow up the observed small INF sample whose 61-mode relative error was 0.5%.
	_, _, InfBlocks, _ = C.build_case(2, 4, 'inf')
	InfTangent = C.HighPrecisionTangent(InfBlocks, 2, 70)
	InfRng = np.random.default_rng(20260813)
	InfDirections, InfReferences = [], []
	for i in range(3):
		Direction = C.project_tangent(InfRng.standard_normal(len(InfBlocks)), InfTangent.A)
		Direction *= .01 / max(abs(Direction))
		InfDirections.append(Direction)
		InfReferences.append(C.fd_gap(InfBlocks, Direction, 2, 1e-4, 70))
	Result['INF_small_sample_followup'] = {'case': 'n2R4INF', 'fd_h': 1e-4, 'fd_dps': 70, 'reference_fd': InfReferences, 'rows': []}
	for Modes in (61, 241):
		Probe = C.SpectralProbe(InfBlocks, Modes, 128)
		QValues = []
		for Direction in InfDirections:
			Lam, Cu, Cw, Diagonal = Probe.pairings(C.block_direction(Direction, Probe.edges))
			QValues.append(C.q_formula(Lam, Cu, Cw, 2, Diagonal))
		Row = {'modes': Modes, 'gauss_order': 128, 'Q': QValues,
			'absolute_error': [abs(Q - F) for Q, F in zip(QValues, InfReferences)],
			'relative_error': [abs(Q - F) / abs(F) for Q, F in zip(QValues, InfReferences)]}
		Result['INF_small_sample_followup']['rows'].append(Row)
		print('INF followup ' + json.dumps(Row), flush=True)
