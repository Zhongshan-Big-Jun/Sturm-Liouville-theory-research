...
R=Path('/mnt/f/LaTeX/BVE research');O=Path('/mnt/f/tools/math-audit-round9-20260923');B=json.loads((O/'baseline.json').read_text());Report='reports/proof-audit-round9-20260923'
...
scripts/_gapn2_k_global_rank2.py
index/tools.json
...
def prepare():
	...
	for prefix in [Report,'research/artifacts/proof-audit-round9-20260923','research/library']:
	...
	paths.add('lean-proof/SL/AuditRound9.lean')
	...
	save(R/manifest,dict(schema='round9-publication/v1',baseline_head=B['head'],scope='Exact authorized round9 changes and immutable evidence only',self_exclusion=manifest,files=rows));paths.add(manifest)
...
def delivery(local=False):
	...
	d=dict(status='PASS',scope='LOCAL_COMMIT_ONLY' if local else 'DELIVERED_TO_BOTH_REMOTES',verified_at_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),baseline_head=B['head'],commit=head,committed_paths=len(expected),remote_heads=remotes,actual_committed_blobs_and_worktree_match_manifest=True,post_commit_status_matches_original_exactly=True,original_untracked_preserved=len(B['untracked']),original_dirty_preserved=list(B['original_dirty']),plugin_source_changed=False,report=Report+'/REPORT.md',formalization_scope='Local finite projection, normalization-kernel and actual matrix/physical-frequency identities only; not infinite spectral/Green analysis, ODE origin, root count, candidate limit or global extrema')
