"""Assemble this author's handoff from actual structured results."""
import ast
import difflib
import hashlib
import io
...
def main():
	...
	Execution = json.loads((BASE / 'execution_manifest.json').read_text())
	if Execution['candidate_sha256_before'] != Hash or Execution['candidate_sha256_after'] != Hash:
		raise RuntimeError('execution candidate identity mismatch')
	for Row in Execution['runs']:
		if Row['exit_code'] != Row['expected_exit_code'] or digest(Path(Row['log'])) != Row['log_sha256']:
			raise RuntimeError('execution outcome or log identity mismatch: ' + Row['name'])
	Tests = [json.loads((BASE / Name).read_text()) for Name in ('tests.json', 'tests_optimized.json')]
	for Test in Tests:
		if Test['failed'] or Test['candidate_sha256'] != Hash:
			raise RuntimeError('test result identity/failure')
	Syntax = []
	for P in BASE.glob('*.py'):
		...
		for Token in tokenize.generate_tokens(io.StringIO(Source).readline):
			if Token.type == tokenize.INDENT and ' ' in Token.string:
				raise RuntimeError('space indentation: ' + str(P))
		Syntax.append({'path': str(P), 'parsed': True, 'indent_tokens_are_tabs': True})
	Inputs = json.loads((BASE / 'input_manifest.json').read_text())
	Preservation = []
	for Name, Original in Inputs['inputs'].items():
		Current = digest(Path(Name))
		Preservation.append({'path': Name, 'before': Original, 'after': Current, 'unchanged': Original == Current})
	Changed = [Row['path'] for Row in Preservation if not Row['unchanged']]
	if any(Name != str(REPO / 'AGENTS.md') for Name in Changed):
		raise RuntimeError('an actual script/dependency/table/audit input changed')
	(BASE / 'source_preservation.json').write_text(json.dumps({'inputs': Preservation,
		'changed_read_only_inputs': Changed, 'author_writes': 'confined to software-author; repository root AGENTS changed outside this author'}, indent=2) + '\n')
	(BASE / 'syntax_checks.json').write_text(json.dumps(Syntax, indent=2) + '\n')
	Original = (REPO / 'scripts/_gapn2_second_variation_probe.py').read_text()
	Patch = ''.join(difflib.unified_diff(Original.splitlines(keepends=True), Candidate.read_text().splitlines(keepends=True),
		fromfile='a/scripts/_gapn2_second_variation_probe.py', tofile='b/scripts/_gapn2_second_variation_probe.py'))
	(BASE / 'replacement.patch').write_text(Patch)
	Cases = []
	TableRows = []
	for Name in ('n2R4SUP', 'n3R4SUP', 'n2R10SUP', 'n2R4INF'):
		D = json.loads((BASE / (Name + '.json')).read_text())
		if D['source_sha256'][str(Candidate)] != Hash:
			raise RuntimeError('case candidate identity mismatch: ' + Name)
		Residual = max(abs(float(Row['direct_fh'])) for Group in ('P1', 'P2') for Row in D[Group])
		AbsError = max(Row['FD'][-1]['absolute_error'] for Row in D['P1'])
		RelError = max(Row['FD'][-1]['relative_error'] for Row in D['P1'])
		Values = [Row['Q'] for Row in D['P2']]
		Signs = '%d negative / %d positive' % (sum(Q < 0 for Q in Values), sum(Q > 0 for Q in Values))
		Cases.append({'case': Name, 'max_P1_P2_direct_residual': Residual, 'P1_max_absolute_fd_error': AbsError,
			'P1_max_relative_fd_error': RelError, 'P2_sample_signs': Signs, 'stationary_residual': D['stationary_residual_max']})
		TableRows.append('| %s | %.3e | %.3e | %.3e | %s |' % (Name, Residual, AbsError, RelError, Signs))
	Sensitivity = json.loads((BASE / 'sensitivity.json').read_text())
	if Sensitivity['candidate_sha256'] != Hash:
		raise RuntimeError('sensitivity candidate mismatch')
	TruncationRows = ['| %d | %.3e | %.3e |' % (Row['retained_modes'], max(Row['P1_absolute_error_vs_mp70_fd_h1e_minus6']),
		max(Row['P1_relative_error_vs_mp70_fd_h1e_minus6'])) for Row in Sensitivity['truncation']]
	Summary = {'role': 'software author, not independent reviewer', 'candidate_sha256': Hash, 'cases': Cases,
		'normal_and_optimized_test_groups': len(Tests[0]['checks']), 'invalid_input_cases_per_mode': Tests[0]['checks'][1]['evidence']['rejected_cases'],
		'execution_commands': len(Execution['runs']), 'sensitivity': Sensitivity}
	(BASE / 'summary.json').write_text(json.dumps(Summary, indent=2, allow_nan=False) + '\n')
	Report = f'''# R9-F01 software-author handoff
...
- 可直接替换 `scripts/_gapn2_second_variation_probe.py` 的候选: `{Candidate}`.
- 候选 SHA-256: `{Hash}`.
...
'''
	...
	print(json.dumps({'candidate_sha256': Hash, 'cases': Cases, 'artifact_identity_checks': 'matched', 'repository_root_agents_changed_externally': bool(Changed)}, indent=2))
