...
def refine_mp_root(Blocks, Guess, Digits):
	Guess = mp.mpf(float(Guess))
	Root = mp.findroot(lambda s: mp_state(Blocks, s)[0], (Guess * mp.mpf('0.99999'), Guess * mp.mpf('1.00001')),
		tol=mp.mpf(10) ** (-(Digits - 10)), maxsteps=80)
	End, Segments, Mass = mp_state(Blocks, Root)
	if Root <= 0 or abs(Root - Guess) > abs(Guess) * mp.mpf('0.001') or abs(End) > mp.mpf(10) ** (-(Digits - 15)) or Mass <= 0:
		raise ArithmeticError('high-precision root refinement failed')
...
class HighPrecisionTangent:
	...
	def __init__(self, Blocks, n, Digits=60):
		...
		with mp.workdps(self.digits):
			...
			self.modes = [refine_mp_root(self.mp_blocks, Guesses[k], self.digits) for k in (n - 1, n)]
			self.integrals = []
...
def fd_second(blocks, dr_blocks, k, h=1e-4, refine=60, dps=60):
	...
	with mp.workdps(dps if dps else 30):
		def Eigenvalue(Sign):
			...
			return refine_mp_root(Precise, Guess, dps)[0] ** 2
...
def build_case(n, R, mode, TablePath=None):
	...
	if R <= 1 or mode not in ('sup', 'inf'):
		raise ValueError('bang-bang case requires R > 1 and mode sup or inf')
...
def run_probe(n=2, R=4.0, mode='sup', Modes=N_MODES + 1, Order=64, Digits=60,
	...
	Result = {'scope': 'software-author finite numerical evidence; no independent audit verdict',
		...
		'limitations': ['finite spectral cutoff; no rigorous tail bound', 'inherited root search is numerical, not certified root enumeration', 'mp and Gauss quadrature are not interval certificates',
	...
	Paths = [Path(__file__).resolve(), TablePath.resolve()] + [Path(sys.modules[Name].__file__).resolve() for Name in
		('_gapn2_symmetry_recon', '_gapn2_jacobian_probe', '_gapn2_jacobian_analytic')]
	...
	if IncludeP3:
		...
		Breaks = tuple(np.r_[Edges - BumpWidth, Edges + BumpWidth])
		for t in range(3):
