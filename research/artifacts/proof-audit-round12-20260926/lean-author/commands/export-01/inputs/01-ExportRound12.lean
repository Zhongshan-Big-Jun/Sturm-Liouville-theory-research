...
elab "#export_round12" : command => do
  ...
  let Names := Env.constants.toList.map Prod.fst |>.filter (fun N =>
    "AuditRound12.".isPrefixOf N.toString) |>.toArray |>.qsort Name.lt
  let mut Records := #[]
  for Name in Names do
    let some Info := Env.checked.get.find? Name | throwError "missing {Name}"
    let Type ← liftTermElabM <| withOptions
      (fun O => O.setBool `pp.all true |>.setBool `pp.universes true |>.setBool `pp.explicit true)
      (do return (← ppExpr Info.type).pretty)
    let Readable ← liftTermElabM <| withOptions
      (fun O => O.setBool `pp.universes true |>.setBool `pp.explicit true |>.setNat `pp.width 120)
      (do return (← ppExpr Info.type).pretty)
    ...
    Records := Records.push <| Json.mkObj [
      ...
      ("type_explicit", toJson Type), ("type_readable", toJson Readable),
      ...
      ("body_dependencies", toJson ((Info.value? (allowOpaque := true) |>.map Expr.getUsedConstants |>.getD #[]).map Name.toString))]
  IO.FS.writeFile "F:/tools/math-audit-round12-20260926/formal-author/declarations.json"
