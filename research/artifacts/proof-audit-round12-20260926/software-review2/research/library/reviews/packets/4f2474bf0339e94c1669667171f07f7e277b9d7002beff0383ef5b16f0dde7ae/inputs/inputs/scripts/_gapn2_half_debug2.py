...
def main():
	...
	Tables = {Boundary: half_spectrum(hb, Boundary, N=80, return_table=True) for Boundary in ('D', 'N')}
	muD = Tables['D'].prefix(80)
	muN = Tables['N'].prefix(80)
	x1 = w[0]
	x2 = w[0] + w[1]
	...
	for (name, mu, pole, bc) in [('D', muD[0], 0, 'D'), ('N', muN[1], 1, 'N')]:
		...
		for yv in [x2 - eps, x2, x2 + eps]:
			sp11 = _spectral_green(hb, mu, pole, bc, x1, yv, N=80, spectrum=Tables[bc])
			sp22 = _spectral_green(hb, mu, pole, bc, x2, yv, N=80, spectrum=Tables[bc])
