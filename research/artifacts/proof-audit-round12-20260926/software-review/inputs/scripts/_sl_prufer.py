"""Indexed DD/DN frequencies for positive piecewise constant densities.

The left endpoint is Dirichlet. Right D targets n*pi; right N targets
(n-1/2)*pi. Targets label the modes independently of the requested count.
...
def _phase(Values, Frequencies):
	...
	Previous = math.sqrt(Values[0, 1])
	for Length, Density in Values:
		RootDensity = math.sqrt(Density)
		Ratio = RootDensity / Previous
		if not math.isfinite(Ratio) or Ratio <= 0:
			raise ArithmeticError('density coordinate change is not resolvable')
		if Ratio != 1:
			Angle = _angle_scale(Angle, Ratio)
		Angle += Frequencies * (RootDensity * Length)
		Previous = RootDensity
	return Angle


def lifted_phase(Blocks, Frequencies):
	"""Lift of atan2(omega*sqrt(last_density)*y, y') at the right endpoint.

	y(0)=0, y'(0)=1. D eigenfrequencies have level n*pi; N have (n-1/2)*pi.
	Block-local angles start at zero and rotate by omega*sqrt(rho)*length; positive
...
def indexed_roots(Blocks, Count, Refine=60, *, RightBoundary='D'):
	"""Return frequencies and per-index numerical phase brackets.

	For total length L and 0<m<=rho<=M, kappa/(L*sqrt(M)) <= omega_n
	<= kappa/(L*sqrt(m)), with kappa=n*pi (D) or (n-1/2)*pi (N).
	Each bracket is bisected against its own boundary-specific phase target.
	...
	"""
	Values = positive_blocks(Blocks)
	if not isinstance(RightBoundary, str) or RightBoundary not in ('D', 'N'):
	...
	Targets = np.pi * (Indices - (0.5 if RightBoundary == 'N' else 0.0))
	...
	Records = [dict(index=i + 1, right_boundary=RightBoundary, frequency=float(Roots[i]), bracket=[float(Lo[i]), float(Hi[i])],
...
def mp_lifted_phase(Blocks, Frequency):
	...
	for Length, Density in Blocks:
		...
		if Previous is not None and RootDensity != Previous:
			Angle = ScaleAngle(Angle, RootDensity / Previous)
		Angle += Frequency * RootDensity * Length
		Previous = RootDensity
	return Angle
