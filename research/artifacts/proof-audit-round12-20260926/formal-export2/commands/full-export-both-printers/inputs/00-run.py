...
def run(Label, Args, Cwd=PROJECT, Inputs=(), Extra=()):
	...
	Record = {'command_id': str(uuid.uuid4()), 'label': Label, 'argv': list(map(str, Args)), 'cwd': str(Cwd),
		'role': 'coordinator-export-repair', 'thread_id': os.environ.get('CODEX_THREAD_ID'),
...
if __name__ == '__main__':
	...
	if Mode == 'version':
		...
		Source = PROJECT / (sys.argv[3] if len(sys.argv) > 3 else 'AuditRound12.lean')
		if not Source.resolve().is_relative_to(BASE):
			raise ValueError('source outside author workspace')
		...
		Result = run(Label, [LEAN, '-R', win(PROJECT), '-o', win(Object), win(Source)], Inputs=[Source, PROJECT / 'lean-toolchain', *[P for D in Extra for P in D.glob("*.olean")]], Extra=Extra)
	elif Mode == 'verify':
		...
		Result = run(Label, [sys.executable, '-B', PLUGIN / 'scripts/verify_lean_project.py', '--project', PROJECT,
			...
			'--output', BASE / 'evidence' / Label], Inputs=[Contract, PROJECT / 'AuditRound12.lean', PROJECT / 'lean-toolchain'])
	else:
