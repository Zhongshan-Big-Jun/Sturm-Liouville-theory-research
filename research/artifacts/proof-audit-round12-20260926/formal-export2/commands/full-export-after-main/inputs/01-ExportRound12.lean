...
elab "#export_round12" : command => do
  ...
  for Name in Names do
    ...
    let PrintedType ← liftTermElabM <| withOptions
      (fun O => O.setNat `pp.maxSteps 10000000 |>.setNat `pp.maxDepth 100000 |>.setBool `pp.deepTerms true |>.setBool `pp.all true |>.setBool `pp.universes true |>.setBool `pp.explicit true)
      (do return (← ppExpr Info.type).pretty)
    let Readable ← liftTermElabM <| withOptions
      (fun O => O.setNat `pp.maxSteps 10000000 |>.setNat `pp.maxDepth 100000 |>.setBool `pp.deepTerms true |>.setBool `pp.universes true |>.setBool `pp.explicit true)
      (do return (← ppExpr Info.type).pretty)
  ...
  IO.FS.writeFile "F:/tools/math-audit-round12-20260926/formal-export2/declarations.json"
