...
if not Rows or len({(X['revision'],X['bundle']) for X in Rows})!=len(Rows):raise RuntimeError('Missing or duplicate current obligations')
Groups={}
for X in Rows:Groups.setdefault(X['target']['location'],[]).append(X)
if not Groups:raise RuntimeError('Empty current card set')
Deps={}
