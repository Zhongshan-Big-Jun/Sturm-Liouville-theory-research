...
R=Path('/mnt/f/LaTeX/BVE research');O=Path('/mnt/f/tools/math-audit-round12-20260926')
...
if len(Notes)!=3:raise RuntimeError('Expected three current version annotations')
for Note in Notes:
...
Summary={'status':'PASS','available':90,'original_withdrawn':1,'verified_current_obligations':len(Releases),'verified_current_cards':len({X['target']['location'] for X in Releases}),'queried_current_cards':len(Targets),'historical_warnings':len(I.get('issues',[])),'query':Q['verdict'],'returned_hits':len(Hits),'targets':[{'location':X['location'],'sha256':X['sha256'],'summary':Hits[X['location']]['summary'],'dependencies':Hits[X['location']]['dependencies']} for X in Targets]}
