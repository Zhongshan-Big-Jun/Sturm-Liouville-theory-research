...
R=Path('/mnt/f/LaTeX/BVE research');O=Path('/mnt/f/tools/math-audit-round12-20260926');B=json.loads((O/'baseline.json').read_text());Report='reports/proof-audit-round12-20260926'
...
scripts/_gapn2_half_problem_probe.py
scripts/_gapn2_half_debug2.py
scripts/_gapn2_half_debug3.py
scripts/_gapn2_sector_decomposition.py
scripts/_gapn2_green_inertia_probe.py
lean-proof/README.md
...
def prepare():
	...
	for prefix in [Report,'research/artifacts/proof-audit-round12-20260926','research/library']:
	...
	paths.update(['lean-proof/SL/AuditRound12.lean'])
	for name in read('checkpoint-files') + read('progress-history-files'):paths.add(name)
	...
	save(R/manifest,dict(schema='round12-audit-publication/v1',baseline_head=B['head'],scope='Round12 indexed DD/DN half spectra, bound pole identity, raw/conjugated sector repairs, scoped analytic and Lean review, correction releases and immutable evidence',self_exclusion=manifest,files=rows));paths.add(manifest)
...
def delivery(local=False):
	...
	for n in B['original_dirty']:require(sha(R/n)==B['tracked'][n],'Original dirty changed '+n)
	def remote(name):
		value=git('-c','http.version=HTTP/1.1','ls-remote','--exit-code',name,'refs/heads/main').decode().strip().split();require(value==[head,'refs/heads/main'],'Remote differs '+name);return name,dict(branch=value[1],sha=value[0],verification='actual git ls-remote')
	remotes={}
	if not local:
		with ThreadPoolExecutor(max_workers=2) as pool:remotes=dict(pool.map(remote,['origin','fork']))
	d=dict(status='PASS',scope='LOCAL_COMMIT_ONLY' if local else 'DELIVERED_TO_BOTH_REMOTES',verified_at_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),baseline_head=B['head'],commit=head,committed_paths=len(expected),remote_heads=remotes,actual_committed_blobs_and_worktree_match_manifest=True,post_commit_status_matches_original_exactly=True,original_untracked_preserved=len(B['untracked']),original_dirty_preserved=list(B['original_dirty']),plugin_source_changed=False,report=Report+'/REPORT.md',formalization_scope='Finite real mirror-sector conjugacy, normalization and odd rank-one algebra only. Independent actual compilation and semantic review; not complete ODE, Python, Green, inertia or interval certification. Existing52 Lean sources preserved.')
