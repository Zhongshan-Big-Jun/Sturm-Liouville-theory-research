...
Root = FilePath('/mnt/f/LaTeX/BVE research')
Out = FilePath('/mnt/f/tools/math-audit-round12-20260926')
...
Snapshot = json.loads(FilePath(Checkpoint['snapshot']).read_text())
...
ArchiveCount = 0
for Name in ['lean-author-archive-manifest.json', 'formal-export2-archive-manifest.json', 'formal-execution-archive-manifest.json', 'coordinator-archive-manifest.json', 'integration/archive-manifest.json', 'integration/document-check-repair/manifest.json', 'integration/publication-repair/manifest.json']:
...
for Review in Reviews:
	...
	Project = FilePath(json.loads(RootFile.read_text())['project']) if RootFile.exists() else Root
