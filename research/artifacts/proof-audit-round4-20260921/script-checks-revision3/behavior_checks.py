...
record('integer coefficient helper remains rational', Module.coefficients('e',4,3)[0]==F(348) and isinstance(Module.coefficients('e',4,3)[0],F), 'P4=348 with integer inputs')
...
for Parity, Wrapper in [('e',Module.even_coeffs),('o',Module.odd_coeffs)]:
	for FloatValue in [1.0,0.1]:
		Expected=Module.coefficients(Parity,4,F(FloatValue))
		Actual=Wrapper(4,FloatValue)
		record('exact wrapper '+Parity+' '+repr(FloatValue),Actual==Expected and all(not Module.sp.sympify(A).has(Module.sp.Float) for A in Actual),'preserved exact represented binary rational')
for Label,Zero in [('Float0',Module.sp.Float(0)),('unevaluated sum',Module.sp.Add(1,-1,evaluate=False)),('unevaluated sin',Module.sp.sin(Module.sp.pi,evaluate=False))]:
	must_reject('symbolic zero baseline '+Label,lambda Z=Zero:Module.reduction_coefficients(1,2,Z,3,4))
Unknown=Module.sp.Symbol('unknown_baseline')
UnknownA,UnknownB=Module.reduction_coefficients(1,2,Unknown,3,4)
record('unknown symbolic nonzero remains caller hypothesis',Module.sp.simplify(UnknownA+11/Unknown)==0 and Module.sp.simplify(UnknownB+8/Unknown)==0,'no unsupported universal symbolic zero decision')
SymbolicFloat=Module.sp.Float('0.1',40)
record('existing SymPy Float semantics retained',Module.even_coeffs(4,SymbolicFloat)==Module.coefficients('e',4,SymbolicFloat),'exact_scalar does not reinterpret supplied symbolic expressions')
Count = 0
with mp.workdps(100):
