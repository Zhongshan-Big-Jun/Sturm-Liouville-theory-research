...
theorem moment_iff_scaled (ε : ℕ) (hε : ε = 0 ∨ ε = 1)
  ...
  · have hn := h n
    change _ = _
    simpa only [scaled_sequence, (scaled_coefficients ε hε c hc n).1,
...
def legendre2 (x : ℚ) : ℚ := (3 * x ^ 2 - 1) / 2

def legendre3 (x : ℚ) : ℚ := (5 * x ^ 3 - 3 * x) / 2

theorem low_mode_representative_differences (x a b : ℚ) :
    legendre2 x / a - (legendre2 x - 1) / a = 1 / a ∧
    legendre3 x / b - (legendre3 x - x) / b = x / b := by
  constructor <;> ring

theorem low_mode_difference_nonzero (a b : ℚ) (ha : a ≠ 0) (hb : b ≠ 0) :
...
theorem remainder_unbounded_on_reciprocals (M : ℚ) :
    ∃ n : ℕ, 0 < (1 : ℚ) / (n + 1) ∧
      M < a6 (1 / ((n : ℚ) + 1)) -
