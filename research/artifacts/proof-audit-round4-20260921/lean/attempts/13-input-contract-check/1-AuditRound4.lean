...
theorem missed_table_parameter (j : ℕ) (hj : 1 ≤ j) :
  ...
  convert table_cancellation (-3 / 4) j h1 h2 using 1
  norm_num
...
def coefficient_step (n : ℕ) (c an am : ℚ) : ℚ :=
  (1 + (4 * (n : ℚ) ^ 2 - 1) / c) * an +
    (2 * (n : ℚ) + 1) / (2 * (n : ℚ) - 3) * (an - am)

theorem remainder_coefficients_from_initials (c : ℚ) :
    coefficient_step 2 c 1 1 = a4 c ∧ coefficient_step 4 c (a4 c) 1 = a6 c := by
  unfold coefficient_step a4 a6
  constructor
  · norm_num
  · by_cases hc : c = 0
    · subst c; norm_num
