...
/-- One cell branch. The affine map sends Start to -1/2 and Start+1/Scale
to 1/2, when Scale is nonzero. Factor is the signed slope ratio. -/
noncomputable def cell_branch (y : ℝ → ℝ) (Scale Start Factor : ℝ)
    (j : ℕ) (x : ℝ) : ℝ :=
  Factor ^ j * y (Scale * (x - Start) - 1 / 2)

theorem cell_branch_has_deriv {y : ℝ → ℝ} {Scale Start Factor x DY : ℝ} {j : ℕ}
    (hy : HasDerivAt y DY (Scale * (x - Start) - 1 / 2)) :
    HasDerivAt (cell_branch y Scale Start Factor j) (Factor ^ j * Scale * DY) x := by
  have hMap : HasDerivAt (fun z : ℝ => Scale * (z - Start) - 1 / 2) Scale x := by
    simpa using (((hasDerivAt_id x).sub_const Start).const_mul Scale).sub_const (1 / 2)
  simpa only [cell_branch, Function.comp_def, mul_assoc, mul_left_comm, mul_comm] using
    (hy.comp x hMap).const_mul (Factor ^ j)

/-- Actual neighboring branches agree in value and first derivative at the
interface. This is a local compatibility statement, not a global gluing theorem. -/
theorem mw_cell_interface {y : ℝ → ℝ} {Scale Start Left Right : ℝ}
    (hScale : Scale ≠ 0) (hLeft : Left ≠ 0)
    (hZeroLeft : y (-1 / 2) = 0) (hZeroRight : y (1 / 2) = 0)
    (hDerivLeft : HasDerivAt y Left (-1 / 2))
    (hDerivRight : HasDerivAt y Right (1 / 2)) (j : ℕ) :
    let Factor := signed_transmission Left Right
    let Join := Start + 1 / Scale
    cell_branch y Scale Start Factor j Join =
      cell_branch y Scale Join Factor (j + 1) Join ∧
    deriv (cell_branch y Scale Start Factor j) Join =
      deriv (cell_branch y Scale Join Factor (j + 1)) Join := by
  dsimp only
  have hMapRight : Scale * (Start + 1 / Scale - Start) - 1 / 2 = (1 : ℝ) / 2 := by
    field_simp
    <;> ring
  have hMapLeft : Scale * (Start + 1 / Scale - (Start + 1 / Scale)) - 1 / 2 =
      (-1 : ℝ) / 2 := by ring
  constructor
  · simp only [cell_branch, hMapRight, hMapLeft, hZeroLeft, hZeroRight, mul_zero]
  · have hDR := cell_branch_has_deriv (Scale := Scale) (Start := Start)
        (Factor := signed_transmission Left Right) (j := j)
        (x := Start + 1 / Scale) (by rw [hMapRight]; exact hDerivRight)
    have hDL := cell_branch_has_deriv (Scale := Scale) (Start := Start + 1 / Scale)
        (Factor := signed_transmission Left Right) (j := j + 1)
        (x := Start + 1 / Scale) (by rw [hMapLeft]; exact hDerivLeft)
    rw [hDR.deriv, hDL.deriv]
    exact mw_signed_slope_matching Left Right Scale j hLeft
