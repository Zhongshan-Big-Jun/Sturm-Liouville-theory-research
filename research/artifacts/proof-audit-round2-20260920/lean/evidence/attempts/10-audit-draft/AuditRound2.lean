...
def signed_transmission (Left Right : ℝ) : ℝ := Right / Left
...
theorem mw_affine_has_deriv {y : ℝ → ℝ} {dy A c d x : ℝ}
  ...
  convert (hy.comp x hAffine).const_mul A using 1 <;> ring
...
theorem second_sine_mode_has_deriv (x : ℝ) :
  ...
  convert (((hasDerivAt_id x).add_const (1 / 2)).const_mul (2 * Real.pi)).sin
    using 1 <;> ring
