...
result.update(current_inputs_match=True,input_count=len(data['inputs']),progress_archive=paths[1])
(O/'checkpoint-result.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n')
dest=R/'research/artifacts/literature-absorption-20260923/integration/checkpoint-result.json';raw=(O/'checkpoint-result.json').read_bytes()
