...
R=Path('/mnt/f/LaTeX/BVE research');O=Path('/mnt/f/tools/math-audit-round11-20260926');B=json.loads((O/'baseline.json').read_text());Report='reports/proof-audit-round11-20260926'
...
tools/second-variation-weighted-eigenvalues.md
tools/README.md
index/tools.json
...
def prepare():
	...
	for prefix in [Report,'research/artifacts/proof-audit-round11-20260926','research/library']:
	...
	paths.update(['docs/SL_cofinite_all_orders.tex','docs/SL_cofinite_all_orders.pdf','lean-proof/SL/AuditRound11.lean','tools/krein-cofinite-closure-all-orders.md'])
	...
	save(R/manifest,dict(schema='round11-audit-publication/v1',baseline_head=B['head'],scope='Round11 residual-Jacobian cross blocks, faithful endpoint differences, independently reviewed full-window cofinite closure, correction releases, scoped local Lean and immutable evidence',self_exclusion=manifest,files=rows));paths.add(manifest)
...
def delivery(local=False):
	...
	d=dict(status='PASS',scope='LOCAL_COMMIT_ONLY' if local else 'DELIVERED_TO_BOTH_REMOTES',verified_at_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),baseline_head=B['head'],commit=head,committed_paths=len(expected),remote_heads=remotes,actual_committed_blobs_and_worktree_match_manifest=True,post_commit_status_matches_original_exactly=True,original_untracked_preserved=len(B['untracked']),original_dirty_preserved=list(B['original_dirty']),plugin_source_changed=False,report=Report+'/REPORT.md',formalization_scope='Local real parity-block/determinant/reflection and explicit boundary-system algebra only; independent actual compilation and semantic review. Not complete ODE, Python, complex/Sobolev/cofinite formalization or interval certification. Existing51 Lean sources preserved.')
