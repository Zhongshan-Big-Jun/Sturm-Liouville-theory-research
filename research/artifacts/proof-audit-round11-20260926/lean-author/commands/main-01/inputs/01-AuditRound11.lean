...
theorem parity_basis_identities (n : ℕ) :
  ...
  have ht : parity_basis n * parity_basis n = (2 : ℝ) • (1 : PairedMatrix n) := by
    simp only [parity_basis, Matrix.fromBlocks_multiply, Matrix.one_mul,
      Matrix.mul_one, Matrix.neg_mul, Matrix.mul_neg, neg_neg]
    ext (i | i) (j | j) <;> simp [Matrix.one_apply, Matrix.fromBlocks, Pi.smul_apply]
    <;> split_ifs <;> norm_num
