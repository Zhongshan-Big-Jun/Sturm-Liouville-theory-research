...
sem={v['name']:v for v in semantic['semantic_nodes']}
for name,node in sem.items():
 verify('semantic_type_constants:'+name,scan(node['type_expression'])==set(node['type_dependencies']))
 ...
 if 'value_expression' in node:
  verify('semantic_body_constants:'+name,scan(node['value_expression'])==set(node['body_dependencies']))
