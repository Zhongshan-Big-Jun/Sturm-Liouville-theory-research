...
run_cmd do
  ...
  for name in names do
    ...
    let displayType ← liftTermElabM <| withOptions
      (fun o => o.setBool `pp.universes true |>.setBool `pp.fullNames true |>.setBool `pp.explicit false)
      do return (← ppExpr info.type).pretty
    let valueText ← match info.value? (allowOpaque := true) with
      | some value =>
        if info.isTheorem then
          pure "[proof term omitted; dependency closure and axioms exported]"
        else
          liftTermElabM <| withOptions (fun o => o.setBool `pp.fullNames true)
    ...
    publicDecls := publicDecls.push <| Json.mkObj [
      ...
      ("actual_type", toJson actualType), ("display_type", toJson displayType), ("value_readback", toJson valueText),
  ...
  IO.FS.writeFile "F:\\tools\\math-audit-round8-20260922\\formal-reviewer\\review-20260921T171659Z-7dddbb5a\\replay-01\\declarations.json" (result.compress ++ "\n")
end Round8Exporter
