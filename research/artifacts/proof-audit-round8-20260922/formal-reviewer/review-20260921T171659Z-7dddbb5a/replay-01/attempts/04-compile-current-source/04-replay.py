...
def host_path(path):
	if re.match(r'^[A-Za-z]:[\\/]',path):
		return Path(os.path.normpath('/mnt/'+path[0].lower()+'/'+path[3:].replace('\\','/')))
	return Path(os.path.abspath(path))


def artifact_paths(modules):
	def module_paths(item):
		base=host_path(item['olean'])
		require(base.is_file(),'Missing imported module '+str(base))
		paths={str(base):base}
		for p in [Path(str(base)+'.private'),Path(str(base)+'.server'),base.with_suffix('.ir')]:
			if p.is_file(): paths[str(p)]=p
		return paths
	paths={}
	with concurrent.futures.ThreadPoolExecutor(max_workers=6) as pool:
		for batch in pool.map(module_paths,modules): paths.update(batch)
	return paths
...
def main():
	...
	public_contract=json.loads((BASE/'public-declaration-contract.json').read_text())
	require(public_contract['source_sha256']==digest(source),'Public contract belongs to another source')
	require(set(expected_names)=={p['declaration'] for p in public_contract['entries'] if p['origin']=='source'},'Source declaration inventory mismatch')
	require(sorted((p['declaration'],p['kind']) for p in public)==sorted((p['declaration'],p['kind']) for p in public_contract['entries']),'Complete public/auxiliary declaration inventory mismatch')
	...
	summary={'status':'AUTHOR_MACHINE_CHECKS_PASSED','run_id':runid,'utc_start':start,'utc_end':datetime.datetime.now(datetime.timezone.utc).isoformat(),
		...
		'authored_theorems':public_contract['authored_theorems'],'authored_definitions':public_contract['authored_definitions'],'compiler_generated_theorems':sum(p['origin']=='compiler_generated' for p in public_contract['entries']),'closure_declarations':len(decls['dependencies']),'external_loaded_modules':len(modules),'inspected_loaded_modules':decls['loaded_module_count'],
