...
def main():
	...
	if args.stage == "supplied":
		...
		for optimized in (False, True):
			...
			run_attempt("certificate-" + mode, files, "certificate.py", ("--output", "results.json"), optimized, no_site=args.no_site)
			for name in certificate.NegativeControls:
				run_attempt("negative-" + name + "-" + mode, files, "certificate.py", ("--negative", name, "--output", "results.json"), optimized, expected_code=1, no_site=args.no_site)
	else:
		run_attempt("bundle-verification", {
