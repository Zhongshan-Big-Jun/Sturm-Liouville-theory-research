...
def run_attempt(label, files, target, arguments=(), optimized=False, expected_code=0):
...
def main():
	...
	parser.add_argument("stage", choices=("supplied", "certificate"))
	...
	if args.stage == "supplied":
		for optimized in (False, True):
			...
			_, record = run_attempt("supplied-" + mode, files, "checks.py", ("results.json",), optimized)
			if record["return_code"] != 0:
				files["continue_supplied.py"] = AuthorRoot / "continue_supplied.py"
				run_attempt("supplied-continuation-" + mode, files, "continue_supplied.py", ("results.json",), optimized, expected_code=1)
		run_attempt("legacy-platform-diagnostics", {
		...
		for optimized in (False, True):
			...
			run_attempt("certificate-" + mode, files, "certificate.py", ("--output", "results.json"), optimized)
			for name in certificate.NegativeControls:
				run_attempt("negative-" + name + "-" + mode, files, "certificate.py", ("--negative", name, "--output", "results.json"), optimized, expected_code=1)
