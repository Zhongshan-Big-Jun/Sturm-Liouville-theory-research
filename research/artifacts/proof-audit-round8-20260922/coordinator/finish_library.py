...
summary_bundle=json.loads((O/'summary-review-dispatch.json').read_text())['bundle']
if V.verify_review_bundle(R,summary_bundle)['verdict']!='APPROVED':raise RuntimeError('Summary review not approved')
summary_repair=json.loads((A/'summary-recovery/promotion.json').read_text())
if summary_repair['status']!='PROMOTED_REVIEWED_DERIVED_SUMMARIES':raise RuntimeError('Derived summaries not promoted')
...
for c in list(cards.values())+list(unchanged.values()):
    ...
    f,body,_=L.read_metadata((R/c['location']).read_bytes())
    if c['location'] not in unchanged and not f.get('summary'):raise RuntimeError('Revised card lacks explicit summary '+c['location'])
    expected=str(f.get('summary') or re.sub(r'\s+',' ',body).strip())[:500]
    if hits[c['location']]['summary']!=expected:raise RuntimeError('Query does not match current source summary '+c['location'])
...
result=dict(status='PASS',available_cards=78,blocked_cards=1,cards=cards,unchanged_renewed_cards=unchanged,versions=len(store['nodes']),correction_events=len(store['events']),current_reviewed_obligations=22,distinct_current_cards=12,release_results=done,renewal_release_results=renewed,derived_summary_repair=summary_repair,historical_superseded_review_warnings=classified,distinct_historical_warning_releases=len({p['release'] for p in classified}),annotations=notes,canonical_modified=False,default_query_verdict=query['verdict'])
