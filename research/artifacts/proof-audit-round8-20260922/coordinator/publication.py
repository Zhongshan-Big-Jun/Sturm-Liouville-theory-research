...
R=Path('/mnt/f/LaTeX/BVE research');O=Path('/mnt/f/tools/math-audit-round8-20260922');B=json.loads((O/'baseline.json').read_text());Report='reports/proof-audit-round8-20260922'
...
docs/SL_gap_n1_inf_limit_proof.pdf
docs/SL_gap_n1_proof.tex
docs/SL_gap_n1_proof.pdf
...
tools/.gitattributes
tools/cot-series-certificate.md
tools/delta-bracketing.md
tools/lemma-A-doubleprime.md
tools/inf-limit-comparison.md
tools/README.md
scripts/README.md
index/tools.json
...
def protect():
	...
	require(git('rev-parse','HEAD').decode().strip()==B['head'],'HEAD changed before publication')
...
def prepare():
	...
	for prefix in [Report,'research/artifacts/proof-audit-round8-20260922','research/library']:
	...
	paths.add('lean-proof/SL/AuditRound8.lean')
	...
	save(R/manifest,dict(schema='round8-publication/v1',baseline_head=B['head'],scope='Exact authorized round8 changes and immutable evidence only',self_exclusion=manifest,files=rows));paths.add(manifest)
...
def delivery(local=False):
	...
	d=dict(status='PASS',scope='LOCAL_COMMIT_ONLY' if local else 'DELIVERED_TO_BOTH_REMOTES',verified_at_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),baseline_head=B['head'],commit=head,committed_paths=len(expected),remote_heads=remotes,actual_committed_blobs_and_worktree_match_manifest=True,post_commit_status_matches_original_exactly=True,original_untracked_preserved=len(B['untracked']),original_dirty_preserved=list(B['original_dirty']),plugin_source_changed=False,report=Report+'/REPORT.md',formalization_scope='Local phase/correction-coefficient/trigonometric-bridge/coverage-witness statements only; not full spectral minmax, implicit-function expansions or T1')
