...
R=Path('/mnt/f/LaTeX/BVE research');O=Path('/mnt/f/tools/math-audit-round5-20260921');B=json.loads((O/'baseline.json').read_text());P=json.loads((O/'protection-result.json').read_text());assert P['status']=='PASS'
Target='reports/proof-audit-round5-20260921/publication-manifest.json';assert not (R/Target).exists()
...
Names.update(['lean-proof/SL/AuditRound5.lean','docs/SL_cofinite_left_definite.tex','docs/SL_cofinite_left_definite.pdf','docs/.gitattributes','lean-proof/SL/.gitattributes','tools/.gitattributes'])
...
Reviews={}
for Name in ['mathematics','dependency-v2','readback','semantic-v2']:
...
Record={'kind':'Reviewed candidate identity recorded before publication, not a post-push receipt','utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'baseline_head':B['head'],'branch':subprocess.check_output(['git','branch','--show-current'],cwd=R,text=True).strip(),'push_order':['origin/main','fork/main'],'review_receipts':Reviews,'protection':P,'path_count_excluding_this_manifest':len(Files),'logical_bytes_excluding_this_manifest':sum(F['bytes'] for F in Files),'largest_file_bytes':max(F['bytes'] for F in Files),'per_file_guard_bytes':100_000_000,'storage_note':'Large exact verifier manifests are preserved under their reviewed paths and hashes. Git stores byte-identical copies as a shared blob. No frozen evidence was reformatted, truncated or replaced with a summary.','files':Files,'post_commit_verification':'F:/tools/math-audit-round5-20260921/LOCAL_COMMIT_VERIFICATION.json','post_push_verification':'F:/tools/math-audit-round5-20260921/DELIVERY.json'}
