...
for name in ['05-complete-root','06-all-declarations','08-positive-controls']:
	text=(OUT/'logs'/(name+'.stdout.txt')).read_text()
	assert 'error:' not in text
	warnings += [{'run':name,'line':line} for line in text.splitlines() if 'warning:' in line]
save('linter-warnings.json',{'warnings':warnings,'interpretation':'One unnecessarySeqFocus style warning; kernel checking and exact-root validation passed. No proof source was changed to suppress it.'})
...
All 59 declaration axiom closures are exported and author-cross-checked. Only propext, Classical.choice and Quot.sound occur. Positive controls pass; a false high-span equality fails; the maintained verifier accepts the complete representative root contract and rejects the false expected equality. Its semantic status is explicitly not_reviewed. {modules['module_count']} loaded modules and {modules['artifact_count']} import artifacts are hash-bound, along with runtime, source snapshots, objects and actual commands/logs. One style-only unnecessarySeqFocus warning remains and is documented in linter-warnings.json. Prior 45 Lean sources plus 3 project configuration files and 32 compiled project objects remain byte-identical.
