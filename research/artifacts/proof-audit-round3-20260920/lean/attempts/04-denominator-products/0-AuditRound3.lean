...
theorem p2_image_moment_rational_identity (m : ℝ)
  ...
  rw [hprev1, hprev3]
  have hden : (2 * m + 1) * (2 * m + 3) ≠ 0 := mul_ne_zero h1 h3
  have hprev : (2 * m - 1) * (2 * m + 1) ≠ 0 := mul_ne_zero h0 h1
  field_simp [h0, h1, h3, hden, hprev]
  ring
...
theorem p2_perturbation_cancellation (m : ℝ)
  ...
  have hden : (2 * m + 1) * (2 * m + 3) ≠ 0 := mul_ne_zero h1 h3
  have hprev : (2 * m - 1) * (2 * m + 1) ≠ 0 := mul_ne_zero h0 h1
  have hdelta : (m - 1) * (2 * m + 3) * (4 * m ^ 2 - 6 * m - 7) ≠ 0 :=
    mul_ne_zero (mul_ne_zero hm h3) hq
  field_simp [hm, h0, h1, h3, hq, hden, hprev, hdelta]
  ring
