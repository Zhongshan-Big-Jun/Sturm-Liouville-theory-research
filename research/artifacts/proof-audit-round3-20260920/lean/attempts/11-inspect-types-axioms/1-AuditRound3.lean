...
theorem p2_image_moment_rational_identity (m : ℝ)
  ...
  rw [hprev1, hprev3]
  have h0' : m * 2 - 1 ≠ 0 := by simpa [mul_comm] using h0
  have h1' : m * 2 + 1 ≠ 0 := by simpa [mul_comm] using h1
  have h3' : m * 2 + 3 ≠ 0 := by simpa [mul_comm] using h3
  field_simp [h0', h1', h3']
  ring
...
theorem p2_perturbation_cancellation (m : ℝ)
  ...
  have h1' : m * 2 + 1 ≠ 0 := by simpa [mul_comm] using h1
  have h3' : m * 2 + 3 ≠ 0 := by simpa [mul_comm] using h3
  have hq' : m * (m * 4 - 6) - 7 ≠ 0 := by
    convert hq using 1
    ring
  field_simp (disch := solve | assumption | (convert hq using 1; ring) | norm_num)
  ring
