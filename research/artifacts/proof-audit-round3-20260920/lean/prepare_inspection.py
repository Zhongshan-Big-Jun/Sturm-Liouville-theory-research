...
SOURCE = Path('/mnt/f/LaTeX/BVE research/lean-proof/SL/AuditRound3.lean')
...
Entries = re.findall(r'^(?:noncomputable )?(def|theorem) (\w+)', Text, re.M)
Names = ['SL.AuditRound3.' + Name for _, Name in Entries]
...
'''
(OUT / 'InspectAuditRound3.lean').write_text(Probe)
(OUT / 'inspection-targets.json').write_text(json.dumps({
	...
	'entries': [{'kind': Kind, 'name': 'SL.AuditRound3.' + Name} for Kind, Name in Entries]
}, indent=2) + '\n')
