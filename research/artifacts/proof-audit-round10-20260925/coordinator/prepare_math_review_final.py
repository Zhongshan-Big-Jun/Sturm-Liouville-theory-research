...
for Name,New in Cards.items():
	...
	for Issue in Entry['state']['issues']:
		if any(X['name']==Name and X['issue']==Issue and X['target']['sha256']==New['sha256'] for X in Done):continue
...
HistoricalDone=Done
Done=[X for X in HistoricalDone if X['target']['sha256']==Cards[X['name']]['sha256']]
save(O/'current-revisions.json',Done)
