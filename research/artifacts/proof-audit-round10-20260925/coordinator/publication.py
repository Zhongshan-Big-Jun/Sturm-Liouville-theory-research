...
R=Path('/mnt/f/LaTeX/BVE research');O=Path('/mnt/f/tools/math-audit-round10-20260925');B=json.loads((O/'baseline.json').read_text());Report='reports/proof-audit-round10-20260925'
...
def prepare():
	...
	for prefix in [Report,'research/artifacts/proof-audit-round10-20260925','research/library']:
	...
	paths.update(['scripts/_sl_prufer.py','scripts/reflection_seeds.py','lean-proof/SL/AuditRound10.lean'])
	...
	save(R/manifest,dict(schema='round10-audit-publication/v1',baseline_head=B['head'],scope='Round10 indexed-root and reflection-sector repairs, reviewed correction propagation, scoped local Lean and immutable evidence only',self_exclusion=manifest,files=rows));paths.add(manifest)
...
def delivery(local=False):
	...
	d=dict(status='PASS',scope='LOCAL_COMMIT_ONLY' if local else 'DELIVERED_TO_BOTH_REMOTES',verified_at_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),baseline_head=B['head'],commit=head,committed_paths=len(expected),remote_heads=remotes,actual_committed_blobs_and_worktree_match_manifest=True,post_commit_status_matches_original_exactly=True,original_untracked_preserved=len(B['untracked']),original_dirty_preserved=list(B['original_dirty']),plugin_source_changed=False,report=Report+'/REPORT.md',formalization_scope='Local reflection and conditional phase-order interfaces only; independent actual compilation and semantic review. Not complete Sturm theory, numerical solver formalization or interval certification. Existing50 Lean sources preserved.')
