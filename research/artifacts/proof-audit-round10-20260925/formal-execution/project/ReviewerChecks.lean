...
theorem zero_level_degenerate :
  ...
  apply bracket_unique_root continuous_id.continuousOn (strictMono_id.strictMonoOn (Set.Ici (0 : ℝ)))
  all_goals norm_num
...
theorem nonconsecutive_example :
  ...
  · intro m
    ...
    · positivity
    · rfl
  · apply indexed_roots_strict_mono (strictMono_id.strictMonoOn (Set.Ici (0 : ℝ))) (fun m => 2*m)
    · intro a b hab
      change 2*a < 2*b
      omega
    · intro m
...
elab "#audit_all " out:str : command => do
  ...
  for name in names.mergeSort (fun a b => a.toString ≤ b.toString) do
    ...
    let typ ← liftTermElabM <| withOptions
      (fun o => o.setBool `pp.explicit false |>.setBool `pp.all false |>.setBool `pp.universes true |>.setBool `pp.deepTerms true)
