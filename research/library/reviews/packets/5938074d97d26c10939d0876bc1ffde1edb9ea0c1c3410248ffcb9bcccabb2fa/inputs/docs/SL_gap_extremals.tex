...
\begin{enumerate}
	...
	\item \emph{$n=1$ 极限 (第 \ref{sec:limit} 节)}: SUP 配置当 $R\to\infty$
...
\medskip
\noindent\textbf{2026-09-21 第七轮范围修订.} 本报告的 $4\pi^2$ 与
$R D\to24.943866\ldots$ 均限于 $n=1$. 对固定 $n\ge1$, 可测盒类
$1\le\rho\le R$ 的谱隙上确界 $S_n(R)$ 满足
$\lim_{R\to\infty}S_n(R)=(n+1)^2\pi^2$; 当前完整证明见
\path{docs/SL_gap_nge2_symmetry_local_proof.tex} 的薄重块构造定理.
这一上确界极限不单独证明全 $R$ 自洽分支的存在或最优性. 本文历史数值表
本轮未整表重算. 对称成对移动的谱隙导数为
$2(\rho_L-\rho_R)f(a)$, 单接口移动则无因子 $2$; 两者坐标不得混用.
...
\subsubsection{$n=1$ SUP 配置: \texorpdfstring{$R \to \infty$}{R-inf} 时 \texorpdfstring{$D \to 4\pi^2$}{D-4pi2} (数值证据, 非证明)}
...
\subsubsection{$n=1$ INF 配置: \texorpdfstring{$R \to \infty$}{R-inf} 时 \texorpdfstring{$D\cdot R \to 24.9439$}{DR-24.9439} (已由定理 A 严格证明)}
