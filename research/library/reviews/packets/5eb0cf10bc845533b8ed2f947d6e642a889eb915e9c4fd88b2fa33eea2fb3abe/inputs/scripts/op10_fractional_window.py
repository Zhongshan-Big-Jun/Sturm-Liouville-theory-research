...
def checked_quad(Fn, *Args, **Kwargs):
	"""Reject failed quadrature instead of reporting a spurious projection norm."""
...
def cos_normsq(k, n):
	...
	if k % 2 or k == 0:
		return 0.0, 1.0
	if k == 2:
		return 4 * (-1)**n / (n * math.pi)**2, 1.0
	Val = checked_quad(lambda x: x**k * math.cos(n * math.pi * x), -1, 1,
...
def sin_normsq(k, Mu):
	...
	if k % 2 == 0 or k == 1:
		return 0.0, NormSq
	Val = checked_quad(lambda x: x**k * math.sin(Mu * x), -1, 1,
		epsabs=1e-11, epsrel=1e-11, limit=200)[0]
	return Val, NormSq
...
def self_check():
	...
	LargePartial = monomial_partial_normsq(2, 0, 1000)
	assert math.isclose(LargePartial, x2_partial_normsq(0, 1000), rel_tol=1e-12)
	assert 0 <= LargePartial <= 2 / 5
	try:
		High = cos_normsq(4, 1000)[0]
	except ArithmeticError:
		pass
	else:
		Exact = 8 / (1000 * math.pi)**2 - 48 / (1000 * math.pi)**4
		assert math.isclose(High, Exact, rel_tol=1e-8, abs_tol=1e-12)
...
def main():
	...
	print("UNSUPPORTED_QUADRATURE: failed or inaccurate estimators raise ArithmeticError; x^2 uses exact coefficients")
