...
class BehaviorChecks(unittest.TestCase):
	...
	def test_exact_zero_rejected_and_tiny_positive_recomputed(self):
		for c in range(1, 11):
			for a in range(1, 11):
				for d in range(1, 11):
		...
		Tiny = Logs.solve_u_log(lambda m: 1, lambda m: math.nextafter(1, 0), 1, 3)[3]
		self.assertAlmostEqual(Tiny, math.log(2**(-53)), delta=1e-13)

	def test_multistep_zero_and_negative_review_regressions(self):
		Cases = [({2: 1.0, 3: 22.0, 4: 1.0}, {2: 0.0, 3: 21.0, 4: 1.0}, F(0)),
			({2: 1.0, 3: 3.0, 4: 1.0}, {2: 0.0, 3: 3071/1024, 4: 1/1024+2**(-54)}, -F(1,2**54))]
		for a, b, Final in Cases:
			A, B = lambda m: a[m], lambda m: b[m]
			self.assertEqual(Exact.solve_u(1,A,B,4)[4], Final)
			with self.assertRaisesRegex(ValueError, 'm=4'):
				Logs.solve_u_log(A,B,1,4)

	def test_multistep_positive_and_nonpositive_ratios(self):
		for Stop in range(4,13):
			for Final in (F(0), -F(1,2**70), F(1,2**70)):
				Ratios = {m: F(1,2**(m%4+1)) for m in range(2,Stop)}
				Ratios[Stop] = Final
				A = lambda m: Ratios[m] if m==2 else Ratios[m]+1/Ratios[m-1]
				B = lambda m: 0 if m==2 else 1
				Actual = Exact.solve_u(1,A,B,Stop)
				if Final<=0:
					self.assertLessEqual(Actual[-1],0)
					with self.assertRaisesRegex(ValueError, f'm={Stop}'):
						Logs.solve_u_log(A,B,1,Stop)
				else:
					LogsActual=Logs.solve_u_log(A,B,1,Stop)
					with mp.workdps(90):
						self.assertAlmostEqual(LogsActual[-1],float(mp.log(to_mp(Actual[-1]))),delta=1e-12)
