...
$\det J=(-1)^n\det A\det B$. 这表示输入输出奇偶扇区交叉映射, 不是 J 分别保持两个子空间;
...
以下为未重跑的历史 N=121 阶梯扫描. 原文件以 Ke/Ko 命名的量实际采用 Kp 的扇区约定; 单个扇区及 H/E 标签必须交换合同后才对应原 K. 整体定性观测的范围仍限这些数值样本:
- SUP (n=2: R 1.05..100; n=3: 1.2..10; n=4: 1.2..10): K_e, K_o 每点正定,
  旧记录声称 λmin(H_o − E_o) + min d > 0 与 λmin(H_e + E_e) + min d > 0 每点成立. 前者不是所写加法分解的充分条件, 该充分性解释撤回; 以下历史余量不认证正确判据
...
- Sherman--Morrison 判据的正确对象: 若 $(K_p)_o=A^p_o-|c_o|(Ew_h)(Ew_h)^T$,
...
- (G1') 仍开放. 本段原标记 STRICT 的 (C1)/(C2)、扇区闭式、E 秩1结构、ε-奇偶代数
  与预解恒等式未在本轮重审. 旧 SUP 减号支配式已撤回为一般充分判据. 对 KpOdd 的有效 Weyl 充分式应使用 H^p_o+E^p_o, 或使用上述负秩一等价条件;
