...
\begin{enumerate}
	...
	\item \emph{第三解与最小解 (定理 \ref{thm:minimal})}: 变差常数 (离散 Wronskian)
		...
		$h^*_j = K(c/4)^j/(j!)^2 \cdot j^{-3}(1+o(1))$ (数值验证,
		一般常数 $K$ 未闭式化; the even $c=1$ anchor is proved separately).
	\item \emph{完整分类 (2026-08-05 补充, 定理 \ref{thm:full})}: 一切
		...
		(25 位数值, EVIDENCE only); the even $c=1$ anchor is STRICT by
		\path{docs/SL_third_order_K1_proof.tex}; 盒式归纳齐次部分封闭
		(退化配置精确排除).
\end{enumerate}
...
递推为严格}, 但一般渐近常数 $K(c)$ 的闭式与数值表为\emph{数值证据}
(未闭式化); $K(1)=e/4$ 的偶次锚点由独立证明文档严格证明; 条目 5 的
\emph{分类定理为严格} (全符号/精确验证), 而
...
$K(1) = e/4$ is now a \textbf{STRICT} theorem for the even recurrence;
the displayed decimal and residual remain historical \emph{EVIDENCE} only
(\path{op13_K1_definitive.py}). See
\path{docs/SL_third_order_K1_proof.tex} for the exact finite sum and limit.
...
严格性: (eq:rhos) 为形式级数 (逐阶匹配), 一般 $K(c)$ 仍为开放问题;
the c=1 even anchor $K(1)=e/4$ is proved in the linked standalone proof.
...
\begin{description}
	\item[源项控制 (残余缺口)] 盒式归纳的齐次部分已封闭 (第 \ref{sec:box} 节),
		...
		前向不变性缺证明.
	\item[最小解的闭式] 一般 $K(c)$ 无闭式; 锚点 $K(0) = \tfrac34$ 仍为
		数值证据. The even c=1 anchor $K(1)=e/4$ is STRICT and has an exact
		finite-sum proof in \path{docs/SL_third_order_K1_proof.tex}. (eq:rhos) 是形式展开,
