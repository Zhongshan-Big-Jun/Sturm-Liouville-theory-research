...
\begin{corollary}\label{cor:crossing}
	...
	the count of zeros of $\bar y$ strictly between its fixed shooting
	endpoint and $x_*$ changes by $\pm 1$ at each crossing.
	For an arbitrary interval with endpoint $x_*$, this count statement
	requires that no zero simultaneously crosses the other endpoint.
\end{corollary}
