...
固定 $n\ge2$ 的优化问题另列为未决; 全序列相邻比值下确界为 $1$,
由常密度的高频序列直接达到极限, 有限指标不取等.
\end{abstract}
...
\begin{equation}
	...
	\quad\Longleftrightarrow\quad
	\sin p=0\quad\text{或}\quad\tan^2 p = s(s+2) = R + 2\sqrt{R} .
\end{equation}
取 $\varphi := \arccos(1/(s+1))$, 则 $\tan^2\varphi = (s+1)^2 - 1 = s^2 + 2s = s(s+2)$,
正根 $\varphi,\pi-\varphi$ 均严格位于 $0<p<\pi$;
另一分支的首个正根为 $\pi$, 因此前两个特征值对应
$p = \varphi$ 与 $p = \pi - \varphi$:
\begin{equation}
...
它比正确式 $(s+2)\varphi/s$ 多出 $\varphi>0$, 并无 $R=4$ 的例外.
本文使用上列正确恒等式.
\end{remark}
...
\begin{remark}[全序列下确界与固定指标问题]
简单谱保证每个正密度及有限 $n$ 的 $\lambda_{n+1}/\lambda_n>1$.
各密度类均包含 $\rho\equiv1$, 此时比值为
$((n+1)/n)^2\to1$, 因而
\[
 \inf_{n\ge1}\inf_{1\le\rho\le R}\lambda_{n+1}/\lambda_n=1,
\]
且有限指标不取等. 这不需要从倍指标下界传递.
$n=1$ 的固定指标下确界为 $\mu(R)$;
固定 $n\ge2$ 的最优值与极值配置仍须另行研究.
\end{remark}
