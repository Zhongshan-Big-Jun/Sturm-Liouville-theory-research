...
同时对指标和密度取下确界时, 精确值为 $1$, 有限指标不取等;
常密度的高频序列即证明这一点. 固定 $n\ge2$ 的优化问题另论.
...
证明须分开首对全局最优性、候选计算与指标传递, 详见证明文档
\texttt{SL\_ratio\_proof.tex}.

\begin{enumerate}
	\item 首对全局极值结构: 密度盒类紧性给出极值;
		首变分、比值单调性、零能量恒等式和界面匹配给出对称平衡配置.
		该步骤为所有容许密度给出全局界, 不使用 MW 倍指标传递来证明自身.
	...
	\item 闭式公式: 对配置 $[1, R, 1]$, 平衡相位条件给出
		...
		其中 $\theta = \arccos(\sqrt{R}/(\sqrt{R}+1))$.
		候选比值为 $((\pi-\theta)/\theta)^2$; 结合第一步全局极值结构才将其识别为 $\nu(R)$.
\end{enumerate}

配套的首对变分证明与 MW Lemma 1--2 重证
(见 \path{SL_mw_lemma_reproof.pdf}) 补全上述逻辑链.
它们使用经典紧自伴谱理论及 Sturm 理论; 历史数值不替代其中任何全局界.
...
简单谱给出每个有限 $n$ 的 $\lambda_{n+1}/\lambda_n>1$.
另一方面, $\rho\equiv1$ 属于所有 $R\ge1$ 的允许密度类,
其相邻比值为 $((n+1)/n)^2\to1$. 因而
$\inf_{n\ge1}\inf_\rho\lambda_{n+1}/\lambda_n=1$, 且有限指标不取等.
$n=1$ 的固定指标下确界为 $\mu(R)$; 固定 $n\ge2$ 的最优值与配置另行研究.
上表历史数值不是上述全序列结论的证明所必需.
\end{remark}
...
$\sqrt{\lambda_1} = 2(s+1)\varphi/s$ 的笔误, 正确恒等式为 $\sqrt{\lambda_1} = (s+2)\varphi/s$.
两式之差为 $\varphi>0$, $R=4$ 也不相等.

周期延拓的正确构造: 对 $R=4$, 将最大首对胞重复 $n$ 次并合并同值胞界,
得到 $[1,4,1,4,\dots,1]$ 共 $2n+1$ 块.
高密度块宽为 $1/(5n)$, 两端低密度块宽为 $2/(5n)$,
内部低密度块宽为 $4/(5n)$. 此构造满足
$\lambda_{2n} = n^2 \lambda_2^{(\text{cell})}$, $\lambda_n = n^2 \lambda_1^{(\text{cell})}$,
故 $\lambda_{2n}/\lambda_n = \lambda_2/\lambda_1 = \nu(4) = 7.48153339$,
这是有符号胞延拓与零点计数的解析恒等式.
旧稿把所有低密度块均设为 $2/(3n+2)$、高密度块设为 $1/(3n+2)$,
混淆了另一固定指标候选族; 撤回对该错误构造的上述精确等式及数值验证声明.
...
\begin{enumerate}
	...
	\item \textbf{归一化必须用权重.} 特征函数需满足 $\int \rho y^2 = 1$,
		忽略权重会得到错误的变分条件.
	\item \textbf{平方因子的来源必须区分.} 正确恒等式是对每个 $k,n\ge1$,
		$\lambda_{kn}(\rho_n)=n^2\lambda_k(\rho_0)$; $n^2$ 来自空间压缩.
		一般不存在 $\lambda_{kn}=k^2\lambda_n$, 连 $k=2$ 也不能这样使用.
...
\begin{enumerate}
	\item 固定 $n\ge2$ 的 $\inf_{\rho}\lambda_{n+1}/\lambda_n$ 及其极值配置.
		全序列下确界 $1$ 已由常密度高频序列解决, 不再列为开放问题.
