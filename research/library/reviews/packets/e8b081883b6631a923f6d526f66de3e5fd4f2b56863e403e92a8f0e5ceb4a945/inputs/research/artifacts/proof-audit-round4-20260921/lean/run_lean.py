...
def run(Name, Args, Sources):
	Log = OUT / 'logs' / Name
	Objects = sorted((OUT / 'build/SL').glob('*.olean')) + sorted((OUT / 'final/build/SL').glob('*.olean'))
	ObjectsBefore = {str(P): digest(P) for P in Objects}
	if Log.with_suffix('.json').exists():
		raise SystemExit('Refusing to overwrite recorded run: ' + Name)
	Env = os.environ.copy()
	Paths = [OUT / 'final/build', OUT / 'build']
	...
	Record = {
		'local_objects_before': ObjectsBefore, 'argv': Args, 'cwd_wsl': str(PROJECT), 'cwd_windows': win_path(PROJECT),
	...
	Record['local_objects_after'] = {str(P): digest(P) for P in Objects if P.exists()}
...
if __name__ == '__main__':
	...
	if Mode in ['build', 'final-build']:
		Artifact = OUT / ('final/build' if Mode == 'final-build' else 'build') / 'SL/AuditRound4.olean'
		Artifact.parent.mkdir(parents=True, exist_ok=True)
		Args = [str(BIN / 'lean.exe'), '-o', win_path(Artifact), 'SL/AuditRound4.lean']
		Sources = [Source] + Configs
	elif Mode == 'local-build':
		Module = sys.argv[3]
		LocalSource = PROJECT / (Module.replace('.', '/') + '.lean')
		Artifact = OUT / 'final/build' / (Module.replace('.', '/') + '.olean')
		Artifact.parent.mkdir(parents=True, exist_ok=True)
		Args = [str(BIN / 'lean.exe'), '-o', win_path(Artifact), Module.replace('.', '/') + '.lean']
		Sources = [LocalSource] + Configs
	elif Mode == 'probe':
		Probe = OUT / sys.argv[3]
		Args = [str(BIN / 'lean.exe'), win_path(Probe)]
		Sources = [Probe, Source] + Configs
	elif Mode == 'deps':
		Args = [str(BIN / 'lean.exe'), '--deps', 'SL/AuditRound4.lean']
		Sources = [Source] + Configs
	elif Mode == 'version':
