...
For displacement d append zero endpoint displacements and let
`delta_w=diff(d)`. With the fixed softmax floor h, any
`t < min_(delta_w_i<0) (w_i-h)/(-delta_w_i)` keeps all widths above h.
Use a safety fraction of this bound and verify the actual reconstructed widths,
