...
\date{历史侦察: 2026 年 8 月 12 日; 程序证据修订: 2026 年 9 月 25 日}
...
\paragraph{第十轮证据修订.}
历史固定网格求根可能漏掉同格的两个低频根, 不能仅凭残差给根编谱序号.
当前引擎改用保留整圈的 Pr\"ufer 相位, 对每个 $j\pi$ 分别括根及细化;
二阶变分的高精度步骤仍保留同一指标. 这不是区间算术认证.
旧 \texttt{asym} 种子代码是 $v\mapsto-Pv$, 并非纯扇区投影.
旧的反对称坐标平面 $Pd=-d$ 实际保持几何镜像对称, 不能据此声称探索了纯破缺扇区.
下文历史统计保留为原试验记录, 不因换引擎而自动取得新认证, 也不由此宣布默认
$R=4$ 的数值全错. 当前重算范围及各自代码身份见
\url{../reports/proof-audit-round10-20260925/REPORT.md}.
解析等变、局部定理及尚未闭合的全局条件按各自证明阅读.
...
\begin{itemize}
	...
	\item (\EVIDENCE) 数值侦察 ($n=2..5$, $R\in\{2,4,10\}$, 两图案): 每个被采样图案仅观测到一类带自洽驻点, 反射对称到 $10^{-11}$ 量级; 其余驻点全部为零宽块退化
...
对给定块配置, 当前以连续提升相位的第 $j\pi$ 个水平求第 $j$ 个 Dirichlet 特征频率 $s_j$; 旧网格扫描已退役,
...
\subsection{保持与破缺反射的两类种子}

记坐标反转为 $P$, 几何反射为 $\mathcal R(x)=1-Px$, 故微分是 $-P$.
在镜像基点 $x_0$ 处, 两个投影是
\[
 d_{\rm preserve}=\tfrac12(v-Pv),\qquad
 d_{\rm break}=\tfrac12(v+Pv).
\]
前者满足 $Pd=-d$, 保持成对镜像; 后者满足 $Pd=d$, 反射将扰动参数 $t$ 变为 $-t$.
旧全索引赋值仅产生 $-Pv$, 其所谓纯扇区统计撤回; 旧拼接 $(a,-Pa)$ 则已属于保持扇区.
当前按两个投影重新生成种子, 零投影重新取样, 过大的步长缩小到严格可行范围,
并在 softmax 往返后检查真实接口位移. 不裁剪、排序再保留原标签.
记录请求/实际步长和扇区残差. 扇区标签只描述初值, 不约束其后完整空间中的优化路径.

\subsection{历史保持对称平面网格 (n=2, R=4)}

旧网格平面 $(\delta_1,\delta_2,-\delta_2,-\delta_1)$ 满足 $Pd=-d$,
因此属于几何保持扇区. 原 17 点/轴、SUP 238 起点、INF 221 起点的日志
\texttt{scripts/\_gapn2\_antigrid\_log.txt} 保留原字节;
其收敛观察不能排除离开对称子空间后的非对称解, 也不构成破缺扇区的独立试验.
本轮不将整个旧网格重新标为通过; 新试验另存可追溯输出.
...
\begin{enumerate}
	...
	\item \textbf{传播约定必须一致}: 对列状态 $(y,y')^{\mathsf T}$ 从左向右
传播, 实际复合是 $P_m\cdots P_1$, 更新为 $M_{\rm new}=P_{\rm new}M_{\rm old}$.
旧叙述把这个更新次序本身称为错误, 不正确; 若改用行状态, 须同时改变矩阵和复合约定.
早期未保留代码变体的具体失败根因不由本轮重新认证.
...
\begin{enumerate}
	...
	\item $R\to\infty$ 的范围更正 (第七轮, 2026-09-21): 原中心质量钉扎
	$D\to4\pi^2$ 只对应 $n=1$, 不适用于本文的一般 $n\ge2$.
	固定 $n\ge1$, 令 $S_n(R)=\sup_{1\le\rho\le R}
	(\lambda_{n+1}(\rho)-\lambda_n(\rho))$. 第七轮新证明给出
	\[
	\lim_{R\to\infty}S_n(R)=(n+1)^2\pi^2.
	\]
	这里是可测盒类 (亦可取有限分块常值类) 的上确界极限,
	不证明从 $R=1$ 出发的某条自洽分支可延拓至全 $R>1$ 或始终最优.
	旧 INF 分支 $D\to0$ 的描述在此保留为未重审的历史趋势,
	不作为全 $R$ 分支结论; (G2) 只约束有限紧 $R$ 区间.
	当前证明见 \path{SL_gap_nge2_symmetry_local_proof.tex};
	修订与实际验收状态见 \path{../reports/proof-audit-round7-20260921/REPORT.md}.
	上述引用需绑定最终版本, 本项修订不是独立验收回执.
\end{enumerate}
...
\begin{enumerate}
	...
	(iv) 必须明确列状态/行状态及乘积约定; 本文列状态更新为
	$M_{\rm new}=P_{\rm new}M_{\rm old}$. 早期未保存变体的失败原因未核实;
	任何"否定性"数值结果都要独立复算.
