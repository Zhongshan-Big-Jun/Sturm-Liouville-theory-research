...
`trace_execution.py` reports the modules present in `sys.modules` after target execution, including its own support imports, with file paths/hashes and exposed versions. It does not filter external imports and is not a complete event log of modules that might have been unloaded. The observed runtime is Python 3.14.4, Linux/WSL2 with glibc 2.43, mpmath 1.3.0 and SymPy 1.14.0. The latter two are used only by the supplied mixed-evidence checker and separate diagnostic. Initial certificate processes also contained the distro startup hooks `_distutils_hack` and `apport_python_hook` under `/usr/lib/python3/dist-packages/`; their actual paths and hashes are recorded. They are not direct certificate imports. A draft overstatement that all observed imports were standard library was corrected; that draft is retained in `development/README-before-import-provenance-correction.md`. Additional normal/-O certificate replays with `-S` disable site initialization and verify the certificate and negative controls without those hooks. No packages were installed or modified.

Isolation here means private input copies, private working directories, `-B` and `PYTHONDONTWRITEBYTECODE=1`, with `-S` explicitly recorded for the additional runs. It is **not** an OS security sandbox, virtual environment, or prevention of arbitrary imports. Code was read before execution, and outputs were directed into the author directory.
...
| `034-certificate-normal`, `049-certificate-optimized` | Exit 0, 19 groups each, with -S | Identical exact results without distro startup hooks |
| `035`-`048`, `050`-`063` | Exit 1 each, with -S | Same 14 intended guard failures in each mode |
| `064-bundle-verification` | Exit 0 | First completed author evidence verification |
| `065-bundle-verification` | Exit 0 | Final author evidence verification including export helper and final replay driver |

Later verification runs, if any, are listed without suppression in `replay_summary.json`. The first bundle verification checked 14 preserved inputs, equality of the four positive certificate results, 56 intended negative failures, 332 directed rational displays and 10 finite arithmetic/display regressions. Its own outer process receipt is written after its output, so `current_run_pending_receipt` refers only to that moment during execution.
...
python3 -B replay.py certificate --no-site
python3 -B replay.py verify --no-site
python3 -B export_records.py
```

The supplied stage requires the already installed mpmath/SymPy and verifies frozen inputs against the coordinator's intake. It never installs them. The certificate itself is portable without the intake or those dependencies. The replay driver is tied to this author's parent round directory and exits nonzero if any captured process disagrees with its expected status. `verify_bundle.py` validates all completed receipts, successful-result equality, directed displays, recorded negative errors, actual provenance and source style. `export_records.py` copies actual verified outputs without modifying any run, then writes `replay_summary.json`, the full `changed_paths.txt`, and `artifact_hashes.json`. The manifest covers every file except itself. These are author evidence records, not an independent review verdict.
