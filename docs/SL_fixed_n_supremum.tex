...
\medskip
\noindent\textbf{标注约定.} 本文档中, \emph{反射对称定理}
(定理 \ref{thm:sym}) 与 $J$-共轭推导是\emph{严格证明};
\emph{平衡定理} (定理 \ref{thm:balance}) 的 $2n$-根计数部分与
\emph{显式闭式} (定理 \ref{thm:closed}) 的 $n \ge 3$ 部分依赖数值验证,
属\emph{数值证据}而非严格证明; 第 4 节全部为\emph{数值证据}
(不构成证明). 严格与数值的边界在第 5 节开放缺口中逐条列明.
