...
\date{2026-08-11 (修订版; 首版 2026-08-05); 2026-09-20 revision}
...
在 $H^2,H^3$ 中解析完备, 并由谱截断得到 $0\le s\le3$ 的结论.
2026-09-20 已撤回原任意整数阶推广, 因 $p_4\notin H^4$. 下述矩必须在所用空间中良定.
论证的核心是\emph{矩跳跃}: 对
...
\begin{theorem}[增长引理, 定量形式]\label{thm:growth}
	设 $c_0 > 0$, 序列 $A_m, B_m$ 满足 $B_m \geq 0$, $A_m - B_m \geq c_0$ ($m \geq 2$).
...
\end{remark}
\begin{remark}[假设的强度]
	定理 \ref{thm:growth} 的统一假设是 $B_m \geq 0$ 且 $A_m - B_m \geq c_0$
	($m \geq 2$), 它保证 $u_m$ 单调不减与 $\varepsilon_m \geq 0$.
	仅假定 $A_m \geq B_m$ 是不够的: 取 $c_0 = 1$, $A_m = B_m = 1$ 时递推解为
	$0, 1, 1, 0, -1, \dots$, 单调性与非负性均失败; 即使 $A_m - B_m \geq 0$
	(但不 $\geq c_0$, 如 $c_0 = 1$, $A_m = 3/2$, $B_m = 1$) 也失败:
	$u_5 = -11/16 < (1/2)^4 = \prod_{k=2}^{5}(A_k - B_k)/c_0$.
	故保证比值连乘所需的最小门槛正是 $A_m - B_m \geq c_0$.
\end{remark}
...
\begin{theorem}[稳定性]\label{thm:stability}
	...
	\end{aligned}
	\end{equation}
	其中 $c_0 > 0$, $B_m, B_m' \geq 0$, $A_m - B_m \geq c_0$, $A_m' - B_m' \geq c_0$ (从而 $\varepsilon_k, \varepsilon_k' \geq 0$),
	且
	\begin{equation}
		\sum_{k=2}^{m} \min(\varepsilon_k, 1) = \omega(\log m),
		\qquad \sum_{k=2}^{m} \min(\varepsilon_k', 1) = \omega(\log m),
	\end{equation}
	这里 $\varepsilon_k = (A_k - B_k - c_0)/c_0 \geq 0$, $\varepsilon_k'$ 同理. 则
	$\{q_n\}$ 在 $H$ 中完备.
\end{theorem}
...
\begin{description}
	...
	\item[假设更正 (2026-08-11, F-001)] 定理 \ref{thm:growth} 与
		\ref{thm:stability} 的陈述原写 $A_m \geq B_m$, 弱于证明实际使用的
		$A_m - B_m \geq c_0$ (Lean 形式化审计 F-001). 本版已统一更正为
		$B_m \geq 0$ 且 $A_m - B_m \geq c_0$; 反例表明弱假设下单调性与乘积
		下界均失败 (见定理 \ref{thm:growth} 后的注). 形式化文件
		\texttt{SL/StabilityGrowth.lean} 一直采用正确假设, 无需改动.
