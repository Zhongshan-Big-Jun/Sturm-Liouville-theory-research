...
for n,R,mode in [(2,4,'sup'),(3,4,'sup'),(2,4,'inf'),(3,4,'inf')]:
 ...
 sd=sector_data(rc,zs,N=200); d=np.array(sd['d']); fac=4*lam_n/lam_np1
 A=np.diag(d/u**2)+fac*M
 print('\n===',n,mode,'===')
 print('lam',lam_n,lam_np1,'fac',fac)
 print('M eig',np.round(np.linalg.eigvalsh(M),6),'norm',np.linalg.norm(M,2))
 print('d/u2',np.round(d/u**2,6),'min',np.min(d/u**2))
 print('A eig',np.round(np.linalg.eigvalsh(A),6))
 print('Ko eig',np.round(np.linalg.eigvalsh(np.array(sd['Ko'])),6))
