...
for n, p in configs.items():
    ...
    ratio = lam[n]/lam[n-1]
    # check conditions: A-regions: |yn|>=|ynp|; a-regions: |yn|<=|ynp|
    pts = [0.0]+jumps+[1.0]
    violA = 0.0; viola = 0.0
    ...
    print(f"n={n}: ratio={ratio:.6f}  max violation A-reg(|yn|<|ynp|): {violA:.2e}  a-reg(|yn|>|ynp|): {viola:.2e}")
    print(f"     jump-condition max diff: {max(jc):.2e}")
