...
def check_keller(jumps, vals, n, label):
    ...
    for i in range(len(pts)-1):
        ...
        isA = (vals[i] == max(vals))
        # max: A-regions |yn| >= |yp|; min: A-regions |yn| <= |yp|
        if isA and not (yn >= yp - 1e-6): ok_A = False
        if not isA and not (yn <= yp + 1e-6): ok_a = False
    print(f"{label}: ratio={lam[n]/lam[n-1]:.8f}  jumps|yn|=|yp|: {ok_jump}  A-reg: {ok_A}  a-reg: {ok_a}")

print("=== Keller/MW conditions for balanced sup configs (a at ends, A middle), n=1 ===")
for R in [1.5, 2.0, 3.0, 4.0, 10.0, 100.0]:
    sR = math.sqrt(R); t = 1.0/(2*sR + 1)
    jumps = [sR*t, sR*t + t]; vals = [1.0, R, 1.0]
    check_keller(jumps, vals, 1, f"R={R:6.1f} sup")

print()
print("=== Keller/MW conditions for balanced inf configs (A at ends, a middle), n=1 ===")
for R in [1.5, 2.0, 3.0, 4.0, 10.0, 100.0]:
    sR = math.sqrt(R); t = 1.0/(2 + sR)
    jumps = [t, t + sR*t]; vals = [R, 1.0, R]
    check_keller(jumps, vals, 1, f"R={R:6.1f} inf")
