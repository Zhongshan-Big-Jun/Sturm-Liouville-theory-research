...
for k in range(2):
    ...
    sol = solve_ivp(lambda t,y: [y[1], -s_star*s_star*rho(t)*y[0]], [0,1], [0.0,1.0],
                    rtol=1e-12, atol=1e-14, dense_output=True, max_step=1e-3)
    nrm, _ = np.trapezoid(rho(sol.t)*sol.y[0]**2, sol.t) if False else (0,0)
    # norm via dense samples
    xs = np.linspace(0,1,20001)
    ys = sol.sol(xs)[0]
    nrm = np.trapezoid(rho(xs)*ys**2, xs)
    yu = sol.sol([u])[0][0]
    print(f"shooting s^2={s_star**2:.8f} vs TM {lam_tm[k]:.8f}; norm={nrm:.8f}; u(u)^2={yu**2/nrm:.8f}")
