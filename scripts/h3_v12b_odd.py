# -*- coding: utf-8 -*-
"""H3 v12b: odd-parity w structure only."""
...
for c in (1.0, 3.0, 10.0):
	...
	print("c={} odd: slope |z^w| [1e5,N]: {:.5f}".format(c, sl2))
	for m in (100, 1000, 10000, 100000, 200000, 300000):
		print("    m={:7d}: z^w={:12.6e}  z^w*sqrt(m)={:12.4f}".format(m, w[m], w[m]*math.sqrt(m)))
	# slope of u and v
	for name, zz in (('u', u), ('v', v)):
		az = np.abs(zz[2:])
		slz, _ = np.linalg.lstsq(np.vstack([np.log(ms), np.ones_like(ms)]).T, np.log(az+1e-300), rcond=None)[0]
		print("    {} slope [2,N]: {:.5f}".format(name, slz))
