# -*- coding: utf-8 -*-
"""#13(i): sequential matched asymptotics for rho_j ~ (c/4)/j^2 (1 + B/j + C/j^2 + ...)."""
from sympy import symbols, Rational, series, expand, solve, factor, Symbol, cancel
import sympy as sp
...
for parity in ('e','o'):
    ...
    rho = (c/4)/j**2*(1 + B/j + C/j**2 + D/j**3 + E/j**4 + F/j**5 + G/j**6)
    rho1 = rho.subs(j, j-1)
    rho2 = rho.subs(j, j-2)
    expr = rho - (a1 + a2/rho1 + a3/(rho1*rho2))
    expr_t = sp.series(expr.subs(j, 1/t), t, 0, 4).removeO()
    sub = {}
    names = [B, C, D, E, F, G]
    # match orders t^{-2}, t^{-1}, t^0, t^1, t^2, t^3
    for k in range(-2, 4):
        coeff = sp.factor(sp.expand(expr_t.coeff(t, k).subs(sub)))
        if coeff == 0:
            print(f"  t^{k}: 0 automatically")
            continue
        sol = solve(sp.Eq(coeff, 0), names[k+2])
        print(f"  t^{k}: coeff = {sp.factor(coeff)}")
        print(f"      -> {names[k+2]} = {sol}")
        if not sol:
            print("  *** no solution; stop"); break
        sub[names[k+2]] = sol[0]
    print("  coefficients:", sub)
