# -*- coding: utf-8 -*-
"""H3 v60: measure local slope of log|z^w| for odd w'=u-2v at large j."""
import math
...
for par in ('e','o'):
    ...
    print("par=%s c=3:"%par)
    for jj in (2000,20000,100000,290000):
        # local slope via central difference of log|z| vs log j
        h=200
        s=(math.log(abs(w[jj+h]))-math.log(abs(w[jj-h])))/(math.log(jj+h)-math.log(jj-h))
        print("   j=%7d: slope=%.5f  j^0.5*z=%.6f  j^1.5*z=%.6f"%(jj,s,abs(w[jj])*math.sqrt(jj),abs(w[jj])*jj**1.5))
