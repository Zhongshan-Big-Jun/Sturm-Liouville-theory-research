# -*- coding: utf-8 -*-
"""analyze_1.py: h = g1 - g2, h', endpoint signs, M-shape from data_g1.json."""
import sys, os, json
import numpy as np
...
def g1inv(rec, target):
    ag = np.array(rec["agrid"]); g1 = np.array([np.nan if x is None else x for x in rec["g1"]])
    mask = ~np.isnan(g1)
    if target < g1[mask][0] or target > g1[mask][-1]:
        return None
...
for Rstr in [f"R={r}" for r in [1.05, 1.2, 2.0, 4.0, 10.0, 100.0, 1000.0, 1e4, 1e5, 1e6]]:
    ...
    # endpoint signs: h(a0) and h(b0) via reflection identities
    ...
    print(f"R={R:9g} fp={fp:.6f} | h(a0)={h_a0:+.3e} h(b0)={h_b0:+.3e} h(fp)={hfp if hfp is None else f'{hfp:+.2e}'} "
          f"| h-crossings on grid: {[round(c,4) for c in crossings]}")
    # h' via finite difference
    hp = np.diff(hvals[:,1])/np.diff(hvals[:,0])
    neg = np.count_nonzero(hp < 0)
    print(f"    h' < 0 at {neg}/{len(hp)} grid intervals; min h'={hp.min():.3e}")
