...
for b in (0.5, 0.7, 0.9, 0.99):
    r1b = float(mp.diff(lambda bb: f_at(a0m, bb, mp.mpf('0.001')), b, 1))
    print("b=%.2f: R1_b(a0,b,eps=0.001) = %.6f   (1st order pred: -eps*Fc0*phi' = %.6f)" % (b, r1b, -0.001*float(mp.acos(mp.mpf(1)/4)/pi*0 + 0)))
