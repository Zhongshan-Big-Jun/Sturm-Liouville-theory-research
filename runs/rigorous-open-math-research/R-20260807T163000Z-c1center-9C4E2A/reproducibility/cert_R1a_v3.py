# -*- coding: utf-8 -*-
"""cert_R1a_v3.py - CERTIFIED: R1_a > 0 on the main box
  a in [a0-0.03, a0+0.03], b in [a0, 0.99], R in [1, 1+eps0].
Root enclosure (elementary, uniform in (a,b)):
  FH: d(lambda_k)/dR = -int_a^b u_k^2 dx,  |.| <= 1  (int u_k^2 drho = 1, rho >= 1)
  => lambda_k(eps) in [k^2 pi^2 - eps, k^2 pi^2]
  => s_k(eps) in [sqrt(k^2 pi^2 - eps0), k*pi] for all (a,b) in the box.
Then R1_a(S1,S2,a,b,R) over cells is a sound enclosure.
Output: cert_R1a_v3.json."""
import mpmath as mp
...
out = dict(status=status, eps0=eps0, Na=Na, Nb=Nb, Ne=Ne,
           S1=[str(S1.a), str(S1.b)], S2=[str(S2.a), str(S2.b)],
           min_R1a_lb=float(min_lb), worst_cell=worst, nfail=nfail, nfail_info=nf_info,
           runtime_s=round(time.time()-t0, 1),
           note="root enclosure via FH |lambda_k'|<=1, elementary; mpmath.iv 200-bit cell-wise")
with open(os.path.join(HERE, "cert_R1a_v3.json"), "w") as f:
    json.dump(out, f, indent=1)
print("written cert_R1a_v3.json, runtime=%.1fs" % (time.time()-t0))
