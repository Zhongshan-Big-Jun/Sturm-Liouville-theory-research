...
eps = mp.mpf('0.001')
print("d_a R1_b at b near 1  (eps=0.001):")
for b in (0.99, 0.999, 0.9999, 0.99999):
    v = float(mp.diff(lambda aa: mp.diff(lambda bb: f_at(aa, bb, eps), b, 1), a0m, 1))
    e = 1-b
    print("  b=%.5f (1-b=%.0e): d_aR1_b = %+.3e   /(1-b)^2 = %.2f" % (b, e, v, v/e**2))
print("R1_b at b near 1 (a=a0):")
for b in (0.99, 0.999, 0.9999):
    v = float(mp.diff(lambda bb: f_at(a0m, bb, eps), b, 1))
    print("  b=%.4f: R1_b = %+.3e   /(1-b)^2 = %.2f" % (b, v, v/(1-b)**2))
print("R1_bb at b near 1:")
for b in (0.99, 0.999, 0.9999):
    v = float(mp.diff(lambda bb: f_at(a0m, bb, eps), b, 2))
    print("  b=%.4f: R1_bb = %+.3e" % (b, v))
