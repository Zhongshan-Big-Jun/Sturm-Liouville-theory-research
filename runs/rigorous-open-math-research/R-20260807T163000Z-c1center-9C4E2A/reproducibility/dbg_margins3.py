# -*- coding: utf-8 -*-
"""dbg_margins3.py - EVIDENCE: E1-right/P0/U' margins, optimized."""
import numpy as np, sys, os
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from fast_local import R1R2
pi = np.pi
...
def sheet_a(b, eps, center=None):
    c = a0 + eps*phi(b) if center is None else center
    lo, hi = c-0.025, c+0.025
    flo = R1R2(lo, b, 1+eps)
    for _ in range(50):
        md = 0.5*(lo+hi)
        if np.signbit(R1R2(md, b, 1+eps)) == np.signbit(flo): lo = md
        else: hi = md
    return 0.5*(lo+hi)

def dR1da(a, b, R, h=1e-6):
    return (R1R2(a+h, b, R) - R1R2(a-h, b, R))/(2*h)
def dR1db(a, b, R, h=1e-6):
    return (R1R2(a, b+h, R) - R1R2(a, b-h, R))/(2*h)
...
for eps in (1e-3, 1e-2, 0.02, 0.05, 0.1):
    ...
    for _ in range(400):
        bn = b + 0.005
        if bn >= 1.0: break
        an = sheet_a(bn, eps, center=a + eps*0.01)
...
for eps in (1e-3, 1e-2, 0.02, 0.05):
    ...
    for b in np.linspace(a0, 0.99, 80):
        a = sheet_a(b, eps)
...
for eps in (1e-3, 1e-2, 0.05):
    vals = []
    for a in np.linspace(a0, min(a0+eps*phi(0.99), b0-1e-9), 30):
        lo, hi = a0, 0.99
        for _ in range(50):
            md = 0.5*(lo+hi)
