# -*- coding: utf-8 -*-
"""h_trace2.py: robust h trace with diagnostics; count zeros."""
...
def branch(a, R, which):
    bb = np.linspace(a+1e-5, 1-1e-5, 80)
    ...
    for i in range(len(bb)-1):
        if np.isfinite(vals[i]) and np.isfinite(vals[i+1]) and vals[i]*vals[i+1] < 0:
            ...
            if which=='g1' and c['va'] > 0: found.append(b0)
            if which=='g2' and c['vb'] < 0: found.append(b0)
    return found[0] if found else None

for R in [1000.0, 3000.0, 10000.0, 30000.0, 100000.0]:
    a0v = np.arccos(0.25)/np.pi; b0v = np.arccos(-0.25)/np.pi
    aa = np.linspace(a0v+2e-4, b0v-2e-4, 51)
    hh = []; miss = 0
    for a in aa:
        g1 = branch(a, R, 'g1'); g2 = branch(a, R, 'g2')
        if g1 is None or g2 is None:
            miss += 1; hh.append((a, None)); continue
        hh.append((a, g1-g2))
    zs = []
    for i in range(len(hh)-1):
        ...
        if h0*h1 < 0:
            ...
            try:
                t0 = brentq(f, 0, 1, xtol=1e-9)
                zs.append(aa[i]+t0*(aa[i+1]-aa[i]))
            except Exception:
                pass
    hvals = [h for (_, h) in hh if h is not None]
    print(f"R={R}: missing={miss}/{len(aa)} h_min={min(hvals):+.6f} h_max={max(hvals):+.6f} zeros={[round(z,6) for z in zs]} n={len(zs)}")
