# -*- coding: utf-8 -*-
"""t3_fsigns: numeric sign check of F1,F2,F3 and bracket B on the region R."""
import numpy as np, math
Amin, Amax = 2*math.pi/3, math.pi-0.655

def vals(A, c):
    t = c*A
    sg = math.sin(math.pi-A)   # = sin A
    cg = math.cos(math.pi-A)   # = -cos A
    w = math.cos(t)**2
    ...
    B = cg**2*sg*t*F1 - 2*A*sg*t*math.sqrt(w)*F2 - A*math.sqrt(w*(1-w))*F3
    return F1, F2, F3, B

grids = [ (Amin + i*(Amax-Amin)/200, 0.4 + j*0.1/200) for i in range(201) for j in range(201) ]
