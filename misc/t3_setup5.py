# -*- coding: utf-8 -*-
"""t3_setup5: fix Gx assembly; verify forms numerically; save pkl."""
import sympy as sp
...
# ---- numerical verification against original (x,c,q) definitions ----
import mpmath as mp
mp.mp.dps = 30
...
def ev(gv, qv):
    Av = mp.pi - gv
    tv = mp.atan(qv*mp.tan(gv))
    sgv, cgv, stv, ctv = mp.sin(gv), mp.cos(gv), mp.sin(tv), mp.cos(tv)
    sv = dict(A=Av, t=tv, sg=sgv, cg=cgv, st=stv, ct=ctv)
    Gv  = float(sp.N(G.subs(sv), 25));  Gcv = float(sp.N(Gc.subs(sv), 25))
    Gxv = float(sp.N(Gx.subs(sv), 25)); uv  = float(sp.N(u.subs(sv), 25))
    xv, cv = Av, tv/Av
    return (Gv-G0s := 0) or (Gv, Gcv, Gxv, uv, float(fG0(xv,cv,qv)), float(fGx0(xv,cv,qv)), float(fGc0(xv,cv,qv)), float(fu0(xv,cv,qv)))
...
for gv, qv in [(0.7,1.5),(0.9,1.2),(1.0,1.5),(0.66,1.9),(1.0472,1.0),(0.85,2.0),(0.75,1.05)]:
    ...
    xv, cv = float(Av), float(tv/Av); qvf = float(qv)
