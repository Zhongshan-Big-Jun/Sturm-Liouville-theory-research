# -*- coding: utf-8 -*-
"""Verify identity modulo s^2+b^2=1, S^2+C^2=1 using groebner reduction."""
import sympy as sp
...
lhs = sp.expand(16*Delta**4*J)
rhs = sp.expand(NJ2sub)
diff = sp.expand(lhs - rhs)
print('diff terms:', len(sp.Add.make_args(diff)))
# reduce by ideal <s^2+b^2-1, S^2+C^2-1>
t0 = time.time()
G = sp.groebner([s**2+b**2-1, S**2+C**2-1], s, b, S, C, order='lex')
print('groebner done in %.1fs' % (time.time()-t0))
r = sp.reduce(diff, G, s, b, S, C)[1]
print('reduced diff:', r)
print('identity holds modulo relations:', r == 0)
