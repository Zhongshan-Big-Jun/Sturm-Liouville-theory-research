# -*- coding: utf-8 -*-
"""Proper iv evaluators for J1_2d, J2_2d and adaptive subdivision counts."""
import mpmath as mp
iv = mp.iv
iv.dps = 50
...
def c1_iv(x, q):
    return iv.atan(1.0/(q*iv.tan(x)))/x

def c2_iv(g, q):
    return iv.atan(q*iv.tan(g))/(mp.pi - g)
...
st, n, lf, e = certify2(J1_2d_iv, mp.mpf('0.8411'), mp.mpf('1.1220'), mp.mpf(1), mp.mpf(2), True, maxdepth=12)
print("J1_2d>0: status=%s boxes=%d leaves=%d err=%s" % (st,n,lf,e[:80]))
st, n, lf, e = certify2(J2_2d_iv, mp.mpf('0.6557'), mp.mpf('1.0472'), mp.mpf(1), mp.mpf(2), False, maxdepth=12)
print("J2_2d<0: status=%s boxes=%d leaves=%d err=%s" % (st,n,lf,e[:80]))
