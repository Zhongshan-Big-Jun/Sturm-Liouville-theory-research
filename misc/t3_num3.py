# -*- coding: utf-8 -*-
"""t3_num3: extract numerators only (no factoring), save to pkl + json summary."""
import sympy as sp
import pickle, json
...
for k,(v,) in comp.items():
    cur = sp.expand(v)
    extra = sp.Integer(1)
    for sym in [p, sg, ct, cg]:
        for _ in range(10):
            nn, dd = sp.fraction(sp.together(cur))
            if dd == 1:
                break
            cur = sp.expand(cur*sym); extra = extra*sym
    ...
    res[k] = {'extra': str(extra), 'nterms': len(monoms), 'deg': poly.total_degree(),
              'pos': len(pos), 'neg': len(neg), 'maxpos': max(pos) if pos else 0, 'minneg': min(neg) if neg else 0,
              'monoms': [list(m) for m in monoms], 'coeffs': [str(c) for c in coeffs]}
    print('%-8s: extra=%-28s n=%d deg=%d pos=%d neg=%d maxpos=%d minneg=%d' % (k, extra, len(monoms), poly.total_degree(), len(pos), len(neg), max(pos) if pos else 0, min(neg) if neg else 0))

with open('misc/t3_num3.json','w',encoding='utf-8') as fh: json.dump(res, fh, ensure_ascii=False)
with open('misc/t3_num3.pkl','wb') as fh: pickle.dump({'res': res, 'comp': {k: v[0] for k,v in comp.items()}}, fh)
print('saved misc/t3_num3.json/.pkl')
