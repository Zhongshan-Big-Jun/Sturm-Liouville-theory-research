# -*- coding: utf-8 -*-
"""t3_num: composed derivative numerators as explicit polynomials; try factoring."""
import sympy as sp
import pickle
...
comp = {
 'dG_dq': (dG_dt, 2),
 'dG_dg': (dG_dg + dG_dt*st*ct/(sg*cg), 3),
 'dGc_dq': (dGc_dt, 3),
 'dGc_dg': (dGc_dg + dGc_dt*st*ct/(sg*cg), 4),
 'dGx_dq': (dGx_dt, 5),
 'dGx_dg': (dGx_dg + dGx_dt*st*ct/(sg*cg), 6),
}
for k,(v,_) in comp.items():
    n, d = sp.fraction(sp.together(v))
    n = sp.expand(n); d = sp.expand(d)
    print('%-8s den = %s' % (k, sp.factor(d)))
