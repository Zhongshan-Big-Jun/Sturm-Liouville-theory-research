# instrumented copy of v2 AllZeroRoots to find the discrepancy
import numpy as np
...
def AllZeroRoots(Rval, coarse=80, verbose=False):
	...
	for a0, b0, m0 in cands[:40]:
		...
		if abs(fa) + abs(fb) > 1e-5:
			continue
		if verbose:
			print("  refined candidate from (%.4f,%.4f): (%.6f,%.6f) va=%.4f vb=%.4f" % (a0, b0, a, b, va, vb))
		dup = False
...
for Rv in [1.2, 10.0]:
	print("R =", Rv)
	rts = AllZeroRoots(Rv, verbose=True)
	print("  total roots:", len(rts))
	for r in rts:
