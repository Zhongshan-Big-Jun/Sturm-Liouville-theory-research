# -*- coding: utf-8 -*-
"""t3_num5: numerators via together(v*denom) then expand."""
import sympy as sp
import pickle, json
...
for k,(v, denom) in specs.items():
    nn = sp.expand(sp.together(sp.expand(v)*denom))
...
with open('misc/t3_num5.json','w',encoding='utf-8') as fh: json.dump(res, fh, ensure_ascii=False)
with open('misc/t3_num5.pkl','wb') as fh: pickle.dump({'res': res, 'comp': {k: v[0] for k,v in specs.items()}, 'denom': {k: v[1] for k,v in specs.items()}}, fh)
print('saved misc/t3_num5.json/.pkl')
