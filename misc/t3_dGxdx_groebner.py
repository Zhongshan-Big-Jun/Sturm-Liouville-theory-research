# -*- coding: utf-8 -*-
"""t3_dGxdx_groebner.py: reduce dGx/dx numerator mod ideal via groebner."""
import sympy as sp, pickle
s, b, S, C, x, th = sp.symbols('s b S C x th', positive=True)
# recompute dGx/dx numerator quickly (as before) and save
Delta = b*s*th + C*S*x
...
dGxdx = sp.cancel(dGxdx)
num = sp.expand(sp.numer(dGxdx))
print('num terms:', len(sp.Add.make_args(num)))
G = [s**2 + b**2 - 1, S**2 + C**2 - 1]
B = sp.groebner(G, s, b, S, C)
